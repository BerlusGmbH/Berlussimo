...
class objekt{
	...
	*/
	...
	hinweis_ausgeben("Es werden alle Einheiten, Mietvertr�ge (inkl. Personen) kopiert<br>");
	$this->dropdown_objekte('objekt_id', 'objekt_id');
	$f->text_feld('Neue Bezeichnung', 'objekt_kurzname', '', 50, 'objekt_kurzname', '');
	$f->text_feld('Vorzeichen f�r Einheiten z.B. E, GBN, II, III', 'vorzeichen', '', 10, 'vorzeichen', '');
	$p = new partners();
	$p->partner_dropdown('Neuen Eigent�mer w�hlen', 'eigentuemer_id', 'eigentuemer_id');
	$f->datum_feld('Datum Saldo VV (letzter Tag vor �bergang)', 'datum_u', '', 'datum_u');
	$f->check_box_js('saldo_berechnen', '1', 'Saldo �bernehmen?', '', '');
	...
	if(!empty($n_objekt_id)){
		...
		if(!is_array($haus_arr)){
			fehlermeldung_ausgeben("Keine H�use im Objekt");
		}else{
			#print_r($haus_arr);
			#die();
			/*Alle H�user durchlaufen und kopieren*/
...
ORDER BY LPAD(EINHEIT_KURZNAME, LENGTH(EINHEIT_KURZNAME), '1') ASC ";
	...
	$result = mysql_query ($db_abfrage);	
		...
		$mv_id = $e->get_mietvertraege_zu($einheit_id, $jahr, $monat, 'DESC'); // OK
	...
	#echo '<pre>';
		...
		$cols = array( 'EINHEIT_KURZNAME1'=>'Einheit', 'TYP'=>"Nutzung", 'MIETER'=>'Mieter', 'MIETER_SEIT'=>'Mieter seit', 'EINHEIT_QM_A'=>'Fl�che m�', 'MIETE_KALT_QM_A'=>'Kaltmiete m�', 'MIETE_KALT_MON_A'=>'Kaltmiete Monat', 'UMLAGEN_A'=>'Nebenkosten');
#	print_r($table_arr);
#	die();
	...
	$pdf->ezTable($arr,$cols,"Mietaufstellung $monatsname $jahr",
array('showHeadings'=>1,'shaded'=>1, 'titleFontSize' => 8, 'fontSize' => 7, 'xPos'=>50,'xOrientation'=>'right',  'width'=>500,'cols'=>array('EINHEIT_KURZNAME1'=>array('justification'=>'left', 'width'=>55),'TYP'=>array('justification'=>'right', 'width'=>50), 'EINHEIT_QM_A'=>array('justification'=>'right', 'width'=>50), 'MIETE_KALT_MON_A'=>array('justification'=>'right', 'width'=>60), 'MIETER_SEIT'=>array('justification'=>'right', 'width'=>50), 'MIETE_KALT_QM_A'=>array('justification'=>'right', 'width'=>50), 'UMLAGEN_A'=>array('justification'=>'right', 'width'=>55))));	
	...
	function pdf_mietaufstellung_m_j($objekt_id, $monat, $jahr){
		...
		if(is_array($arr)){
			...
			$cols = array( 'EINHEIT_KURZNAME1'=>'Einheit', 'TYP'=>"Nutzung", 'MIETER'=>'Mieter', 'MIETER_SEIT'=>'Mieter seit', 'EINHEIT_QM_A'=>'Fl�che m�', 'MIETE_KALT_QM_A'=>'Kaltmiete m�', 'MIETE_KALT_MON_A'=>'Kaltmiete Monat', 'UMLAGEN_A'=>'Nebenkosten', 'MIETE_BRUTTO'=>'BruttoM', 'MWST'=>'MWSt', 'ZAHLUNGEN'=>'Zahlung');
			#	print_r($table_arr);
			#	die();
			...
			$pdf->ezTable($arr,$cols,"$oo->objekt_kurzname - Mietaufstellung $monatsname $jahr",
...
echo "<tr><td>$einheit_kn</td><td>$nutzung</td><td>$mieter</td><td>$einzug</td><td>$qm</td><td>$km_qm</td><td>$km_mon</td><td>$nk</td><td>$wm</td><td>$mwst</td><td>$zahlung</td></tr>";
	...
	$result = mysql_query ("SELECT HAUS_STRASSE FROM HAUS WHERE HAUS_AKTUELL='1' && OBJEKT_ID='$objekt_id' GROUP BY HAUS_STRASSE");
		while ($row = mysql_fetch_assoc($result)) $haeuser_array[] = $row;
...
function pdf_checkliste($objekt_id){
	...
	include_once('classes/class_bpdf.php');
		...
		$pdf->ezSetDy(-20);
		$pdf->ellipse(60,$pdf->y-10,10);
		$pdf->ezText("FENSTERB�NKE UND BRIEFK�STEN", 12);
		
		$pdf->ezSetDy(-20);
		$pdf->ellipse(60,$pdf->y-10,10);
		$pdf->ezText("GEL�NDER / HANDLAUF", 12);
		...
		$pdf->ezSetDy(-20);
		$pdf->ellipse(60,$pdf->y-10,10);
		$pdf->ezText("M�LLPLATZ FEGEN", 12);
		
		
		$pdf->ezSetDy(-20);
		$pdf->ellipse(60,$pdf->y-10,10);
		$pdf->ezText("T�RSCHLIESSER KONTROLLIEREN / EINSTELLEN", 12);

		
		$pdf->ezSetDy(-20);
		$pdf->ellipse(60,$pdf->y-10,10);
		$pdf->ezText("SPERM�LLBESEITIGUNG", 12);
		...
		echo "<h3>Vor der Objekteingabe muss ein Partner (Eigent�mer) erstellt worden sein.</h3>"; 
		...
		$this->get_objekt_infos($objekt_id);
		$f = new formular;
		$f->erstelle_formular("Objekt $this->objekt_kurzname �ndern", NULL);
    	...
    	$f->send_button("submit_obj1", "Objekt �ndern");
		$f->ende_formular();
		}
		...
		$objekte_array = array ();
		while ($row = mysql_fetch_assoc($result)) $objekte_array[] = $row;
	...
	var $haus_stadt;
		*/
		...
		$o->get_objekt_infos($objekt_id);
			...
			$f->text_feld("Gr�sse in qm", "qm", "", "10", 'qm','');
...
ORDER BY OBJEKT_KURZNAME, HAUS_STRASSE, HAUS_NUMMER, EINHEIT_LAGE";
	$cols = array('OBJEKT_KURZNAME'=>"Objekt", 'HAUS_STRASSE'=>"Strasse"
,'HAUS_NUMMER'=>'Nr','EINHEIT_KURZNAME'=>'Einheit', 'TYP'=>'Typ','EINHEIT_LAGE'=>'Lage', 'EINHEIT_QM'=>'Fl�che m�', 'MIETER'=>'Mieterinfos', 'MIETER'=>'Mieter', 'KONTAKT'=>'Kontakt');
#	print_r($table_arr);
#	die();
$pdf->ezTable($my_arr,$cols,"Alle Einheiten",
array('showHeadings'=>1,'shaded'=>1, 'titleFontSize' => 8, 'fontSize' => 7, 'xPos'=>50,'xOrientation'=>'right',  'width'=>500,'cols'=>array('OBJEKT'=>array('justification'=>'left', 'width'=>65),'HAUS_NUMMER'=>array('justification'=>'right', 'width'=>30), 'EINHEIT_QM'=>array('justification'=>'right', 'width'=>30))));	
...
$numrows = mysql_numrows($result);
	...
	function get_einheit_typ_arr(){
		...
		$typ_array = array ();
		while ($row = mysql_fetch_assoc($result)) $typ_array[] = $row;
		return $typ_array;		
		}
...
function get_mietvertraege_zu2($einheit_id, $jahr, $monat){
	...
	function get_einheit_haus($einheit_id){
		...
		function form_einheit_aendern($einheit_id){
			...
			$f = new formular;
			$f->erstelle_formular("Einheit �ndern", NULL);
			...
			$f->text_feld("qm", "qm", "$e->einheit_qm_k", "10", 'qm','');
			$h = new haus;
    		echo "<br>";
    		$h = new haus;
    		$h->dropdown_haeuser_2('Haus w�hlen', 'haus_id', 'haus_id', $e->haus_id);
    		...
    		$f->send_button("submit_einheit", "�nderung speichern");	
			$f->ende_formular();
			}else{
...
class mietvertrag extends einheit{
		...
		$anzahl = mysql_numrows($result);
		$this->anzahl_mietvertraege_gesamt = $anzahl; //auch abgelaufene MV
		}
		...
		$anzahl = mysql_numrows($result);
		$this->anzahl_personen_im_vertrag = $anzahl; //Alle Personen im MV
		}
		...
		while ($row = mysql_fetch_assoc($result)) $aktuelle_mietdaten[] = $row;
		...
		$ausgabe_str .= "<hr><hr>Monatlich f�llig: <b>$summe �</b></div>\n";
		#return $summe;
		return $ausgabe_str;
		}
		...
		while ($row = mysql_fetch_assoc($result)) $alle_zahlungen[] = $row;
...
$numrows = mysql_numrows($result);
    	...
    	$form->text_feld("F�llig am", "faellig_am", $datum_heute, "10");
    	$form->text_bereich("Kurzbeschreibung", "kurzbeschreibung", "", "50", "10");
    	$form->send_button("submit_rechnung1", "Rechnung speichern");
    	$form->hidden_feld("option", "rechnung_erfassen1");
    	$form->ende_formular();
    	...
    	$kasse_info->dropdown_kassen('Kasse als Empf�nger', 'empfaenger_id', 'empfaenger');
...
GROUP BY RECHNUNGEN.BELEG_NR DESC"); 
		*/	
		...
		while ($row = mysql_fetch_assoc($result)) $my_array[] = $row;
		echo "<table class=sortable>\n";
		#echo "<tr class=feldernamen><td>Erfassungsnr</td><td>TYP</td><td>Rech.Nr</td><td>F�lig</td><td>Von</td><td>An</td><td width=80>Netto</td><td width=80>Brutto</td><td width=80>Skonto</td></tr>\n";
		echo "<tr><th>Erfassungsnr</th><th>TYP</th><th>Rech.Nr</th><th>F�lig</th><th>Von</th><th>An</th><th>Netto</th><th>Brutto</th><th>Skonto</th></tr>\n";
...
WHERE RECHNUNGEN.BELEG_NR = RECHNUNGEN_POSITIONEN.BELEG_NR && RECHNUNGEN.AKTUELL='1' && RECHNUNGEN_POSITIONEN.AKTUELL='1' GROUP BY RECHNUNGEN.BELEG_NR ASC");
		...
		if($numrows>0){
		/*Seitennavigation mit  Limit erstellen*/
		echo "<table><tr><td>Anzahl vollst�ndige Rechnungen: $numrows</td></tr><tr><td>\n";
...
GROUP BY RECHNUNGEN.BELEG_NR ORDER BY BELEG_NR DESC $navi->limit");
		...
		echo "<tr class=feldernamen><td>BNr</td><td>R-Datum</td><td>E-Datum</td><td>F�lig</td><td>Von</td><td>An</td><td width=80>Netto</td><td width=80>Brutto</td><td width=80>Skonto</td></tr>\n";
	...
	$r_datum = date_mysql2german($my_array[$a][RECHNUNGSDATUM]);
		...
		$numrows1 = mysql_numrows($result);
		echo "<table><tr><td>Anzahl aller unvollst�ndig erfassten Rechnungen: $numrows1</td></tr><tr><td>\n";
		/*Seitennavigation mit  Limit erstellen*/
...
FROM RECHNUNGEN, RECHNUNGEN_POSITIONEN
WHERE RECHNUNGEN.BELEG_NR = RECHNUNGEN_POSITIONEN.BELEG_NR && RECHNUNGEN.AKTUELL='1' && RECHNUNGEN_POSITIONEN.AKTUELL='1' GROUP BY RECHNUNGEN.BELEG_NR ASC");
		while ($row = mysql_fetch_assoc($result)) $rechnungen_mit_positionen[] = $row;
...
WHERE RECHNUNGEN.BELEG_NR = RECHNUNGEN_POSITIONEN.BELEG_NR && RECHNUNGEN.AKTUELL='1' && RECHNUNGEN_POSITIONEN.AKTUELL='1' GROUP BY RECHNUNGEN.BELEG_NR ASC");*/
	...
	$r_datum = date_mysql2german($my_array[$a][RECHNUNGSDATUM]);
		...
		function rechnung_kontierung_aufheben($belegnr){
			mysql_query("UPDATE  `KONTIERUNG_POSITIONEN` SET AKTUELL='0' WHERE `BELEG_NR` ='$belegnr'") or die('Kontierungsaufhebung nicht m�glich');
			return true;
		}
...
##########OBJEKTE###################
/*Ein Array mit Objekten erstellen, dieser wird nachher mit Unterarrays gef�llt*/
		$objekte = new objekt;
...
#################H�USER######################		
/*Aus dem Kontierungspool werden alle Positionen aller H�user in ein Array geschoben*/
...
/*Empf�ngerinformationen d.h. Empf�nger der neuen Rechnung aus Pool
 * bei Objekt -> Eigent�mer
 * Haus-Eigent�mer ...usw*/
		...
		$rechnung_vzweck = "Rechnung f�r Einheit $einheit";
		}
		if($kostentraeger_typ == 'Haus'){
		$rechnung_vzweck = "Rechnung an den Eigent�mer f�r ein Haus";
		}
		if($kostentraeger_typ == 'Objekt'){
		$rechnung_vzweck = "Rechnung an den Eigent�mer f�r das Objekt $this->objekt_name";
		}
		...
		*/
		...
		$f->hidden_feld('RECHNUNG_EMPFAENGER_TYP', "$kostentraeger_typ");
			...
			$mwst_satz_in_prozent = nummer_punkt2komma($this->mwst_satz_der_position($belegnr, $position)); //aus Beleg infos holen //
...
echo "</td><td valign=bottom>$kostentraeger</td></tr>";
			...
			echo "<tr><td colspan=9><hr></td></tr>";
  			echo "<tr><td colspan=8 align=right>Netto ausgew�hlte Positionen</td><td id=\"g_netto_ausgewaehlt\"></td></tr>";
  			echo "<tr><td colspan=8 align=right>Brutto ausgew�hlte Positionen</td><td id=\"g_brutto_ausgewaehlt\"></td></tr>";
...
function rechnung_schreiben_positionen_wahl_LETZTE($kostentraeger_typ, $kostentraeger_id, $positionen, $aussteller_typ, $aussteller_id){
		...
		*/
		...
		$f->hidden_feld('RECHNUNG_EMPFAENGER_TYP', "$kostentraeger_typ");
			...
			$mwst_satz_in_prozent = $this->mwst_satz_der_position($belegnr, $position); //aus Beleg infos holen //
			$kontierung_id = $positionen[$a][KONTIERUNG_ID];
			$kontierung_dat = $positionen[$a][KONTIERUNG_DAT];
			$f->hidden_feld("positionen[$a][kontierung_dat]", "$kontierung_dat");
...
$resultat = mysql_query($db_abfrage) or
		...
		weiterleiten_in_sec("?daten=rechnungen&option=positionen_erfassen&belegnr=".$letzte_belegnr."", 2);//Positionseingabe		
...
$ae_link = "<a href=\"?daten=rechnungen&option=position_aendern&belegnr=$belegnr&pos=$position\">�ndern</a>";
...
echo "Den Buchungsbetrag betrag bitten wir auf folgendes Konto zu �berweisen:<br><br>";
...
$link_grunddaten_aendern = "<a href=\"?daten=rechnungen&option=rechnungsgrunddaten_aendern&belegnr=$belegnr\"><b>Grunddaten �ndern</b></a>";


$status_kontierung = $this->rechnung_auf_kontierung_pruefen($belegnr); //vollst�ndig, unvollst�ndig oder falsch
...
#$this->rechnung_als_unvollstaendig($belegnr); //unvollst�ndig und nicht zugewiesen, 2xstatus aufgehoben hat auswirkungen auf buchung, da keine buchung eine unvollst�ndigen rechnung m�glich ist
}
...
echo "<td valign=top><b>ERFASSUNGSNR:</b><br>Rechnungsnr:<br>Rechnungsdatum:<br>Eingangsdatum:<br>F�llig am:";
...
$link_details_hinzu ="<a href=\"?daten=details&option=details_hinzu&detail_tabelle=RECHNUNGEN&detail_id=$belegnr\">Lieferschein hinzuf�gen</a>";

if($this->rechnungstyp=='Schlussrechnung'){
$link_teilrg_hinzu ="<a href=\"?daten=rechnungen&option=teil_rg_hinzu&beleg_id=$belegnr\">Teilrechnung hinzuf�gen</a>";	
}else{
	$link_teilrg_hinzu ='';
}
...
echo "<p id=\"rechnungsfaellig_u\">F�llig: $this->faellig_am</p>\n";
...
echo "<tr><td valign=top><b>$rechnungs_aussteller_name</b><br>$rechnungs_aussteller_str $rechnungs_aussteller_hausnr<br><br>$rechnungs_aussteller_plz $rechnungs_aussteller_ort </td><td valign=top>Noch nicht definiert<br>Kostentr�ger ist ein /-e $kostentraeger_typ</td><td valign=top><b>Ursprungsdaten<hr></b>Belegnr:<br>Rechnungsnr: <br>Rechnungsdatum: <br>Eingangsdatum: <br>F�llig am: </td><td valign=top><br><hr>$this->belegnr<br><b>$this->rechnung_id</b><br> $this->rechnungsdatum<br> $this->eingangsdatum<br> <b>$this->faellig_am</b></td></tr>\n";
...
echo "<tr><td colspan=9><b>Kostentr�ger w�hlen</b>\n";
$this->dropdown_kostentreager_typen();
echo "</td></tr>\n";
$kt = new kontenrahmen;
echo "<tr><td colspan=9><b>Kostentr�ger w�hlen</b>\n";
$kt->dropdown_kontenrahmen('Kontenrahmen w�hlen', 'kontenrahmen', 'kontenrahmen', '' );
echo "</td></tr>\n";
echo "<tr><td colspan=9><b>F�r die Kontierung w�hlen Sie bitte alle zusammenh�ngenden Positionen aus!!!</b></td></tr>\n";
...
function kontierungstabelle_anzeigen($beleg_nr, $positionen_arr, $kostentraeger_typ){
	$this->rechnung_grunddaten_holen($beleg_nr);
	#print_r($this);
	$form = new mietkonto; // nur f�r die formularerstellung
	$kontenrahmen = new kontenrahmen; // nur kontoliste dropdown
	$rechnung = new rechnung; // f�r rechnungsmethoden
	#$this->rechnung_grunddaten_holen($beleg_nr);
    ...
    echo "<table>\n";
    echo "<tr class=feldernamen><td>Pos</td><td>Artikelnr</td><td>Bezeichnung</td><td>Menge</td><td>LP </td><td>EP</td><td align=right>Rabatt</td><td align=right>MWSt</td><td width=90>Gesamt</td><td>Konto</td><td>Kostentr�ger</td><td>Weiter verwenden</td><td>Verwendung im Jahr</td></tr>\n";
...
/* Wegen der Rechnungskontierung muss hier der Kontenrahmen f�r alle angezeigt werden*/
...
echo "<tr><td>$anzahl_pos_zu_kontierung von $anzahl_pos_beleg Positionen aus Beleg $beleg_nr haben Sie ausgew�hlt.</td></tr>\n";
	echo "</table>\n";
}//end function
...
$msg = 'Den Rechnungsbetrag  bitten wir auf das unten genannte Konto zu �berweisen.';
$colspan=12;
$colspan1 = $colspan -2;
}
if($this->rechnungstyp == "Gutschrift"){
$msg = 'Den Rechnungsbetrag werden wir auf Ihr Konto �berweisen.';
$colspan=10;
$colspan1 = $colspan -2;
}
if($this->rechnungstyp == "Buchungsbeleg"){
$colspan=12;
$colspan1 = $colspan -2;
$msg = "Den Buchungsbetrag bitten wir auf folgendes Konto zu �berweisen:<br><br>";
$msg .= "Empf�nger: $geld_konto_info->konto_beguenstigter<br>";
...
echo "</div>"; //ende div_positionen f�r druck	
...
$empfangs_geld_konto = '0'; // NULL BEI KASSE	
}
if(!isset($empfangs_geld_konto)){
echo "Kein Geldkonto ausgew�hlt";	
}else{
...
echo "Dem Beleg $belegnr wurde die Kontonummer des Rechnungsausteller hinzugef�gt<br>\n";
...
/*Wenn nicht die gesamte Menge in Rechnung gestellt wurde*/
	...
	$this->kontierungs_menge_anpassen_dat($dat,$kontierungs_menge);//menge die in Rechnung gestellt wurde
	}
...
function suche_rechnung_form(){
    	...
    	echo "</td><td>\n";
    	$partner_arr=$partner->partner_dropdown('Aussteller w�hlen', 'aussteller', 'aussteller');
    	echo "</td></tr><tr><td>\n";
    	$form->radio_button("suchart", "empfaenger", 'Ausgestellt an');
    	echo "</td><td>\n";
    	$partner_arr=$partner->partner_dropdown('Empf�nger w�hlen', 'empfaenger', 'empfaenger');
    	echo "</td></tr><tr><td>\n";
    	$form->radio_button("suchart", "partner_paar", 'Partnerpaar ausw�hlen');
    	echo "</td><td>\n";
...
echo "<table class=rechnungen>\n";
		echo "<tr class=feldernamen><td>ErfNr</td><td>RNr</td><td>TYP</td><td>R-Datum</td><td>F�llig</td><td>Von</td><td>An</td><td width=60>Netto</td><td width=60>Brutto</td></tr>\n";
		#print_r($my_array);
		for($a=0;$a<count($my_array);$a++){
		$belegnr = $my_array[$a]['BELEG_NR'];
...
$form->erstelle_formular("Ausgew�hlt: Lager", NULL);	
...
echo "<th >LFDNR</th>";
echo "<th scopr=\"col\">Rechnungsempf�nger</th>";
...
class blaettern{
var $aktuelle_seite; // zeigt an wo man ist
var $limit; // abfrageteil mit z.B. limit 0,1
...
var $kredit_institut;
	...
	$numrows = mysql_numrows($result);
		...
		*/
		...
		$link_aendern= "<a class=\"table_links\" href=\"?daten=geldkonten&option=gk_aendern&gk_id=$konto_id\">GK �ndern</a>";
		if($zaehler==1){
 ...
 */
 ...
 /*Leerst�nde nach objekt beispiel f�r objekt 4
...
function lager_auswahl_liste($link){
	...
	$mieten = new mietkonto;
	$mieten->erstelle_formular("Lager ausw�hlen...", NULL);
	if(isset($_SESSION[lager_id])){
 	$lager_bezeichnung = $this->lager_bezeichnung($_SESSION[lager_id]);
 	echo "<p>&nbsp;<b>Ausgew�hltes Lager</b> -> $lager_bezeichnung";
	}else{
 	echo "<p>&nbsp;<b>Lager ausw�hlen</b>";
	}
	echo "<p class=\"objekt_auswahl\">";	
	$lager_arr = $this->lager_in_array();
