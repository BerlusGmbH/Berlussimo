 ...
 * 
 * @filesource   $HeadURL: http://192.168.2.52/svn/berlussimo_1/tags/02.11.2010 - Downloadversion 0.27/classes/class_mietvertrag.php $
 * @version      $Revision: 15 $
...
$link="?daten=mietvertrag_raus&mietvertrag_raus=mietvertrag_neu";
...
function mv_personen_anzeigen($arr){
	#print_r($arr);
	for($a=0;$a<count($arr);$a++){
...
function mv_personennamen_als_string($arr){
	#print_r($arr);
	$mystring = '';
	for($a=0;$a<count($arr);$a++){
	$person_id= $arr[$a][PERSON_MIETVERTRAG_PERSON_ID];
...
$numrows = mysql_numrows($result);
	...
	$nn =$row['PERSON_NACHNAME'];
	$vn = $row['PERSON_VORNAME'];
	$mystring = $mystring."$nn $vn";
	}
	if($numrows>1)
	$row = mysql_fetch_assoc($result);
	$nn =$row['PERSON_NACHNAME'];
	$vn = $row['PERSON_VORNAME'];
	$mystring = $mystring."$vn $nn, ";
	}
	return $mystring;
}	
...
$bg->objekt_auswahl_liste('?daten=mietvertrag_raus&mietvertrag_raus=mietvertrag_neu');
}
...
$resultat = mysql_query($db_abfrage) or
           die(mysql_error()); // aktuell auf 0 gesetzt
...
$resultat = mysql_query($db_abfrage) or
           die(mysql_error()); // personen zu MV gelöscht bzw auf 0 gesetzt
...
#$teilnehmer_objekt_arr[$teilnehmer_arr_z]['MIETER'][$i]['NACHNAME'] = $person_info->person_nachname;
...
$link="?daten=mietvertrag_raus&mietvertrag_raus=ls_teilnehmer";
$this->objekt_auswahl_liste($link);
$form = new mietkonto;
...
/*echo "<table>";
echo "<tr class=\"feldernamen\"><td colspan=3>Objekt $teilnehmer_arr['OBJEKT_KURZNAME']</td></tr>";
echo "<tr class=\"feldernamen\"><td>EINHEIT</td><td>MIETER</td><td>OPTIONEN</td></tr>";
*/
$zaehler=0;
...
$link_deaktivieren = "<a href=\"?daten=mietvertrag_raus&mietvertrag_raus=ls_teilnehmer_deaktivieren&mietvertrag_id=$mv_id\">Deaktivieren</a>";
...
$link="?daten=mietvertrag_raus&mietvertrag_raus=ls_teilnehmer_inaktiv";
$this->objekt_auswahl_liste($link);
$form = new mietkonto;
...
$anzahl_mieter =$teilnehmer_arr[$a]['MIETER_ANZAHL'];
$erster_mieter = $teilnehmer_arr[$a]['MIETER'][0]['NACHNAME'].' '.$teilnehmer_arr[$a]['MIETER'][0][VORNAME];
$link_aktivieren = "<a href=\"?daten=mietvertrag_raus&mietvertrag_raus=ls_teilnehmer_aktivieren&mietvertrag_id=$mv_id\">Aktivieren</a>";
$zaehler++;
...
function anzahl_eingezogene_mieter($objekt_id, $jahr, $monat){
	$mon_laenge = strlen($monat);
if($mon_laenge==1){
	$monat = '0'.$monat;
	}
...
function anzahl_ausgezogene_mieter($objekt_id, $jahr, $monat){
	$mon_laenge = strlen($monat);
	if($mon_laenge==1){
		$monat = '0'.$monat;
	}
...
$auszug_arr = $this->alle_ausgezogene_mieter_arr($jahr, $monat);
	$link = "<a href=\"?daten=mietvertrag_raus&mietvertrag_raus=alle_letzten_auszuege_pdf&monat=$monat&jahr=$jahr\">Ansicht als PDF</a>";
	echo $link;
	if(is_array($auszug_arr)){
	$anzahl_auszuege = count($auszug_arr);
	$e = new einheit;
		...
		$link_pdf = "<a href=\"?daten=mietvertrag_raus&mietvertrag_raus=abnahmeprotokoll&einheit_id=$einheit_id&mv_id=$mv_id\"><img src=\"css/pdf.png\"></a>";
...
$monat_name = monat2name($monat);
	...
	$e = new einheit;
		...
		#echo "</table>";
	#$pdf->ezTable($pdf_tab);
	$cols = array('EINHEIT'=>"EINHEIT", 'MIETER'=>"MIETER", 'AUSZUG'=>"AUSZUG", 'ABNAHME'=>"ABNAHME", 'KAUTION'=>"KAUTION");
...
#print_r($table_arr);
#die();
$cols = array('EINHEIT'=>"Einheit", 'MIETER'=>"Mieter",'LAGE'=>"Lage",'QM'=>"m²",'EINZUG'=>"Einzug",'AUSZUG'=>"Auszug"
,'BETRIEBSKOSTEN'=>"BK", 'HEIZKOSTEN'=>"HK", 'KM'=>"Kaltmiete");
$datum_h = date("d.m.Y");
	
$pdf->ezTable($table_arr,$cols,"Nebenkostenhochrechnung für das Jahr $jahr vom $datum_h",
array('showHeadings'=>1,'shaded'=>1, 'titleFontSize' => 8, 'fontSize' => 7, 'xPos'=>50,'xOrientation'=>'right',  'width'=>740,'cols'=>array('EINHEIT'=>array('justification'=>'left', 'width'=>70),'MIETER'=>array('justification'=>'left'), 'EINZUG'=>array('justification'=>'right','width'=>46),'AUSZUG'=>array('justification'=>'right','width'=>46),'BETRIEBSKOSTEN'=>array('justification'=>'right','width'=>50), 'HEIZKOSTEN'=>array('justification'=>'right','width'=>50), 'KM'=>array('justification'=>'right','width'=>50))));
...
$pdf->ezTable($table_arr,$cols,"Soll - Nebenkosten/Kaltmiete $datum_von_ber_a bis 31.12.$jahr",
...
$pdf->ezTable($table_arr,$cols,"Soll - Nebenkosten/Kaltmiete für das Jahr $jahr",
		array('showHeadings'=>1,'shaded'=>1, 'titleFontSize' => 8, 'fontSize' => 7, 'xPos'=>50,'xOrientation'=>'right',  'width'=>500,'cols'=>array('EINHEIT'=>array('justification'=>'left', 'width'=>75),'MIETER'=>array('justification'=>'left', 'width'=>175), 'EINZUG'=>array('justification'=>'right','width'=>46),'AUSZUG'=>array('justification'=>'right','width'=>46),'BETRIEBSKOSTEN'=>array('justification'=>'right','width'=>60), 'HEIZKOSTEN'=>array('justification'=>'right','width'=>60), 'KM'=>array('justification'=>'right','width'=>60))));
...
#$db_abfrage ="SELECT * FROM `MIETVERTRAG` WHERE (MIETVERTRAG_VON<='$datum_bis' && (MIETVERTRAG_BIS='0000-00-00' OR (MIETVERTRAG_BIS BETWEEN '$datum_von' AND '$datum_bis'))) && `MIETVERTRAG_AKTUELL` = '1' && MIETVERTRAG.EINHEIT_ID IN (SELECT EINHEIT_ID FROM EINHEIT, HAUS, OBJEKT WHERE EINHEIT.HAUS_ID=HAUS.HAUS_ID && HAUS.OBJEKT_ID='$objekt_id' && EINHEIT.EINHEIT_AKTUELL='1') ORDER BY EINHEIT_ID ASC";
...
function pdf_istmieten($arr, $von, $bis){
	...
	$cols = array('EINHEIT'=>"EINHEIT", 'MIETER'=>"MIETER",'EINZUG'=>"EINZUG",'AUSZUG'=>"AUSZUG",'SUMME'=>'SUMME');
	...
	$pdf->ezTable($arr,$cols,"Bruttomieteinnahmen $von-$bis",
array('showHeadings'=>1,'shaded'=>0, 'titleFontSize' => 8, 'fontSize' => 7, 'xPos'=>50,'xOrientation'=>'right',  'width'=>500,'cols'=>array('SUMME'=>array('justification'=>'right', 'width'=>70))));
...
#  hinweis_ausgeben("Saldenliste mit Vormieter für $objekt_name wurde erstellt<br>");
ob_clean(); //ausgabepuffer leeren
$cols = array('EINHEIT'=>"Einheit", 'MIETER'=>"Mieter",'EINZUG'=>"Einzug",'AUSZUG'=>"Auszug"
,'BETRIEBSKOSTEN'=>"Betriebskosten $jahr", 'HEIZKOSTEN'=>"Heizkosten $jahr");
$datum_h = date("d.m.Y");
	
$pdf->ezTable($table_arr,$cols,"Nebenkostenhochrechnung für das Jahr $jahr vom $datum_h",
array('showHeadings'=>1,'shaded'=>0, 'titleFontSize' => 8, 'fontSize' => 7, 'xPos'=>50,'xOrientation'=>'right',  'width'=>500,'cols'=>array('EINHEIT'=>array('justification'=>'left', 'width'=>75),'MIETER'=>array('justification'=>'left', 'width'=>175), 'EINZUG'=>array('justification'=>'right','width'=>50),'AUSZUG'=>array('justification'=>'right','width'=>50),'BETRIEBSKOSTEN'=>array('justification'=>'right','width'=>75), 'HEIZKOSTEN'=>array('justification'=>'right','width'=>75))));
...
function alle_aktuellen_mvs_arr(){
	 ...
	 *  SELECT *, DATE(NOW())
FROM `MIETVERTRAG`
WHERE (MIETVERTRAG_BIS >= DATE( NOW( )) OR MIETVERTRAG_BIS = '0000-00-00')

&& `MIETVERTRAG_AKTUELL` = '1'
 ORDER BY `MIETVERTRAG`.`MIETVERTRAG_AKTUELL` ASC
*/
	 
}


function alle_ausgezogene_mvs_arr(){
	/*
	 *  SELECT *  , DATE( NOW( ) )
FROM `MIETVERTRAG`
WHERE (
MIETVERTRAG_BIS <= DATE( NOW( ) )
AND MIETVERTRAG_BIS != '0000-00-00'
) && `MIETVERTRAG_AKTUELL` = '1'
ORDER BY `MIETVERTRAG`.`MIETVERTRAG_BIS` ASC
	 */
}
...
/*Abfrage alle Mvs aus einem bzw bis zu einem Jahr
 * SELECT `MIETVERTRAG_ID`,`MIETVERTRAG_VON`,`MIETVERTRAG_BIS`,`EINHEIT_ID` 





FROM `MIETVERTRAG` WHERE `MIETVERTRAG_AKTUELL`='1' 
&& DATE_FORMAT(MIETVERTRAG_VON,'%Y') <= '2008'
...
IF(DATE_FORMAT(MIETVERTRAG_VON, '%Y') < '2009', '2009-01-01', MIETVERTRAG_VON) AS BERECHNUNG_VON,

IF(DATE_FORMAT(MIETVERTRAG_BIS, '%Y') = '2009', MIETVERTRAG_BIS, '2009-12-31') AS BERECHNUNG_BIS,

DATEDIFF(IF(DATE_FORMAT(MIETVERTRAG_BIS, '%Y') = '2009', MIETVERTRAG_BIS, '2009-12-31'), IF(DATE_FORMAT(MIETVERTRAG_VON, '%Y') < '2009', '2009-01-01', MIETVERTRAG_VON))+1 AS TAGE




FROM `MIETVERTRAG` WHERE `MIETVERTRAG_AKTUELL`='1' 
&& DATE_FORMAT(MIETVERTRAG_VON,'%Y') <= '2009' && (DATE_FORMAT(MIETVERTRAG_BIS,'%Y') >='2009' OR DATE_FORMAT(MIETVERTRAG_BIS,'%Y') ='0000') && EINHEIT_ID='7'
*/
