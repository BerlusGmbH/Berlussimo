...
function buchungsauswahl($mietvertrag_id){
	...
	echo "<br>";
	$this->hidden_feld("MIETVERTRAG_ID", "$mietvertrag_id");
	$this->text_feld("Anderer Zahlbetrag (€):", "ZAHLBETRAG", 
"$summe_forderung_monatlich1", "6");
	echo "<br>";
		...
		//2008-2007 = 1
		 	...
		 	for($i=$diff_in_jahren;$i>=0;$i--){
		 		...
		 		if($jahr == $jahr_einzug){
		 			...
		 			if($a<10){
		 			$datum_jahr_arr = array("monat"=>"0$a","jahr"=>"$jahr");
		 			}else{
		 			$datum_jahr_arr = array("monat"=>"$a","jahr"=>"$jahr");
		 			}
...
protokollieren('MIETBUCHUNGEN', $mietbuchung_dat, $mietbuchung_dat);
		...
		while ($row = mysql_fetch_assoc($result)) $buchungen_arr[] = $row;
...
return $EINHEIT_KURZNAME;	
		...
		while ($row = mysql_fetch_assoc($result)) $mv_arr[] = $row;
...
function zeitraum_generieren($monat_von, $jahr_von, $monat_bis, $jahr_bis){
		...
		if($diff_in_jahren==0){
			...
			for($monat=$monat_von;$monat<=$monat_bis;$monat++){
				if($monat<10){
				$datum_jahr_arr = array("monat"=>"0$monat","jahr"=>"$jahr_bis");
				}else{
						$datum_jahr_arr = array("monat"=>"$monat","jahr"=>"$jahr_bis");
						}
		 	$monate_arr[] = $datum_jahr_arr;
			} //end for
		 }//end else
