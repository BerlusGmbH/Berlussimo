...
function get_iban_bicALTFALSCH($konto_nr, $blz, $land='DE'){
	...
	if($land=='DE'){
		...
		if($numrows){
			...
			#die();
	// BBAN: 8-stellige BLZ plus 10-stellige Kontonummer (ggf. fÃ¼hrende Nullen hinzufÃ¼gen)
	...
	// 	- In der ASCII-Tabelle befinden sich GroÃŸbuchstaben an den Positionen 65 bis 90. Es wird also die ASCII-Position der Buchstaben ausgelesen und das Ergebnis minus 64 plus 9 (=55) gerechnet
	//- An die vierstellige LÃ¤nderkennzahl werden zwei Nullen angehÃ¤ngt.
	$land_num = strval(ord(substr($land, 0, 1)) - 55).strval(ord(substr($land, 1, 1)) - 55)."00";
	
	// Modulus 97 der aneinandergehÃ¤ngten BBAN und LÃ¤nderkennzahl ergibt die PrÃ¼fzahl als Teil der IBAN:
...
$arr = $this->mandat_nutzungen_arr($m_ref);
 ...
 function umlautundgross($wort)
   {
      $tmp = strtoupper($wort);
      $suche = array('Ä', 'Ö', 'Ü', 'ß','ä', 'ö', 'ü', 'ß', 'é', '&', '*', '$', '%', '€', '<BR>');
      $ersetze = array('AE', 'OE', 'UE', 'SS','AE', 'OE', 'UE', 'SS', 'E', '+', '.', '.', '.', 'EUR', ' ');
...
function form_ds_kontoauszug($ds){
	...
	$f->ende_formular();
		...
		fehlermeldung_:ausgeben("Keine Daten");
	}
}
