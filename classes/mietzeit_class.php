 ...
 * 
 * @filesource   $HeadURL: http://192.168.2.52/svn/berlussimo_1/tags/02.11.2010 - Downloadversion 0.27/classes/mietzeit_class.php $
 * @version      $Revision: 15 $
...
$numrows = mysql_numrows($result);
		if($numrows>0){
		while ($row = mysql_fetch_assoc($result)) $my_arr1[] = $row;	
		}	
...
function letzte_vorauszahlung_summe($kos_typ, $kos_id, $jahr, $kostenkat){
	...
	WHERE MIETENTWICKLUNG_AKTUELL = '1' && `KOSTENTRAEGER_TYP` = '$kos_typ' && `KOSTENTRAEGER_ID` = '$kos_id' && KOSTENKATEGORIE = '$kostenkat' && ( DATE_FORMAT(ENDE,'%Y') >= '$jahr'
	OR ENDE = '0000-00-00' ) && DATE_FORMAT(ANFANG, '%Y') <= '$jahr'
	ORDER BY `NEW_ENDE` DESC ";*/# alt falsch
...
$cols = array('DATUM'=>"Datum", 'BEMERKUNG'=>"Bezeichnung", 'BETRAG'=>"Betrag",'SALDO'=>'Saldo');
$seit_monat = monat2name($drucken_m);
$pdf->ezTable($table_arr,$cols,"Mietkontenblatt seit $seit_monat $drucken_j",
array('showHeadings'=>1,'shaded'=>0, 'titleFontSize' => 8, 'fontSize' => 7, 'xPos'=>50,'xOrientation'=>'right',  'width'=>500, 'rowGap'=>1, 'cols'=>array('DATUM'=>array('justification'=>'right', 'width'=>50),'BEMERKUNG'=>array('justification'=>'left', 'width'=>300), 'BETRAG'=>array('justification'=>'right','width'=>75), 'SALDO'=>array('justification'=>'right','width'=>75))));
ob_clean(); //ausgabepuffer leeren
...
function pdf_tabelle_mieterbelastung($kto, $mv_id, $pdf){
	...
	if($numrows){
		...
		$pdf->ezNewPage();
		$cols = array('DATUM'=>"Datum", 'TXT'=>"Buchungstext", 'BETRAG'=>"Betrag",'KOS_BEZ'=>'ZUWEISUNG', 'KOS_TYP'=>'TYP');
		$pdf->ezTable($pdf_table,$cols,"Mieterbelastung",
				array('showHeadings'=>1,'shaded'=>0, 'titleFontSize' => 8, 'fontSize' => 7, 'xPos'=>50,'xOrientation'=>'right',  'width'=>500, 'rowGap'=>1, 'cols'=>array('DATUM'=>array('justification'=>'right', 'width'=>50),'BEMERKUNG'=>array('justification'=>'left', 'width'=>300), 'BETRAG'=>array('justification'=>'right','width'=>75), 'SALDO'=>array('justification'=>'right','width'=>75))));
