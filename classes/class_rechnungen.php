...
$p_id = $_SESSION['partner_id'];
	...
	$sum =0;
	echo "<table class=\"sortable\">";
	echo "<tr><th>DATUM</th><th>RNR</th><th>TYP</th><th>EMPFÃ„NGER</th><th>KURZINFO</th></th><th>BETRAG</th></tr>";
	while($row=mysql_fetch_assoc($result)){
	$sum += $row['NETTO'].
	$rn = $row['RECHNUNGSNUMMER'];
	$netto = $row['NETTO'];
...
$f->send_button("submit_r_gd", "Ã„nderung speichern");
...
echo "<label for=\"$id\">$beschreibung</label>\n";
		...
		echo "<option value=\"mÂ²\">mÂ²</option>\n";
		echo "<option value=\"mÂ³\">mÂ³</option>\n";
...
$f->hidden_feld("option", "rechnung_buchen_gesendet");
	...
	echo "NICHT VOLLSTÃ„NDIG ERFASST/KONTIERT!";
}
...
if($anzahl){
	...
	echo "<p id=\"link_rechnung_klein_bl\">BLAU - NICHT BEZAHLT + GELDEINGANG BESTÃ„TIGT</p>";
	echo "<p id=\"link_rechnung_klein_gr\">GRÃœN - BEZAHLT + GELDEINGANG BESTÃ„TIGT</p>";
	echo "</div>";
...
/*Spaltendefinition*/		
$cols = array('POSITION'=>"<b>POS.</b>", 'ARTIKEL_NR'=>"<b>ARTIKELNR</b>",   'BEZ'=>"<b>BEZEICHNUNG</b>", 'MENGE'=>"<b>MENGE</b>",  'PREIS'=>"<b>NETTO</b>",   'MWST_SATZ'=>"<b>MWST</b>", 'RABATT_SATZ'=>"<b>RABATT</b>", 'SKONTO'=>"<b>SKONTO</b>", 'GESAMT_NETTO'=>"<b>GESAMT</b>");
...
$tableoptions = array
    ( 
      ...
      'showHeadings' => 1,               // zeig Ãœberschriften der spalten    
      ...
      'cols' => array
      (
		'POS'         =>array('justification'=>'left', 'width'=>25),         
		'ARTIKEL_NR'         =>array('justification'=>'left','width'=>70),
		'BEZ'         =>array('justification'=>'left','width'=>130),
         'MENGE'         =>array('justification'=>'right','width'=>50),
         'PREIS'         =>array('justification'=>'right','width'=>50),
         'MWST_SATZ'         =>array('justification'=>'right','width'=>50),
         'RABATT_SATZ'         =>array('justification'=>'right','width'=>50),
         'SKONTO'         =>array('justification'=>'right','width'=>50), 
         'GESAMT_NETTO'         =>array('justification'=>'right','width'=>50),
...
$pdf->ezText("$p->partner_ort, $this->rechnungsdatum", 10, array('justification'=>'right'));
if($this->rechnungstyp!='Angebot'){
$pdf->ezText("Fällig: $this->faellig_am  ", 10, array('justification'=>'right'));
}
...
$pdf->ezText("$kurzbeschreibung", 10, array('justification'=>'full'));
...
/*Zahlungshinweis bzw mit freudlichen Grüssen usw vom Aussteller*/
...
$pdf->ezSetDy(-10); //abstand
$pdf->ezText("<b>Rechnungsaufstellung</b>", 9, array('justification'=>'full'));		
...
$pdf->ezText("$r_hinweis", 8, array('justification'=>'full'));	
...
#$pdf->ezText("Rechnungsaufstellungn", 9, array('justification'=>'full'));	
	...
	if($numrows){
		...
		$pdf_tab[$z]['NETTO'] = '=========================';
 		...
 		$cols = array('RDATUM'=>"<b>Datum</b>", 'RNR'=>"<b>Rechnungsnr</b>",   'NETTO'=>"<b>Betrag Netto</b>", 'MWST'=>"<b>Betrag MwSt</b>", 'BRUTTO'=>"<b>Betrag Brutto</b>");
...
$tableoptions = array
    ...
    'shaded' =>0,                     // shaded:    0-->Zeile 1 & Zeile 2 --> weiss  1-->Zeile 1 = weiss    Zeile 2= grau    2-->Zeile 1= grauA      Zeile 2= grauB
      ...
      'cols' => array
      (
		'RDATUM'         =>array('justification'=>'left', 'width'=>90),         
		'RNR'         =>array('justification'=>'left','width'=>100),
		'NETTO'         =>array('justification'=>'right','width'=>120),
        'BRUTTO'         =>array('justification'=>'right','width'=>120),
        'MWST'         =>array('justification'=>'right','width'=>100),
...
function u_pool_edit($kos_typ,$kos_id, $aussteller_typ, $aussteller_id){
	$r = new rechnung();
		...
		for($a=0;$a<$anz;$a++){
			...
			$f->button_js('btn_back_p', 'Züruck', $js_back);
    ...
    ( 
     ...
     'xPos'=>410,
      ...
      'colGap' => 1,  
        'cols' => array
      (
		'LFDNR'         =>array('justification'=>'left'),         
		'EMPFAENGER'         =>array('justification'=>'left'),
		'KURZTEXT'         =>array('justification'=>'left'),
		'BRUTTO'         =>array('justification'=>'right'),
		'GUT_RET'         =>array('justification'=>'right'),
        'DATUM'         =>array('justification'=>'left'),
		'SKONTO'         =>array('justification'=>'right')         
     )        
   ) ; 
