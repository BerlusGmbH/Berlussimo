...
if(file_exists("classes/class_partners.php")){
...
if(file_exists("classes/class_urlaub.php")){
...
function zeiterfassung(){
	...
	$this->gewerk_finden($_SESSION['benutzer_id']);
...
$result = mysql_query ("SELECT ZETTEL_ID, STUNDENZETTEL.BENUTZER_ID, BESCHREIBUNG, ERFASSUNGSDATUM, benutzername, STUNDENSATZ, STUNDEN_PW  FROM STUNDENZETTEL JOIN  BENUTZER ON (STUNDENZETTEL.BENUTZER_ID = BENUTZER.benutzer_id) WHERE ZETTEL_ID='$id' && AKTUELL = '1' LIMIT 0,1");
	...
	$this->stunden_pro_woche = $row['STUNDEN_PW'];
...
function stundenzettel_anzeigen($id){
	...
	echo "<hr><b>ï¿½BERSICHT: MITARBEITER -> $this->st_benutzername<hr>Gesamtarbeitszeit in Stunden: $this->gesamt_azeit_std<br>";
	...
	#$link_zettel2beleg = "<a href=\"?daten=zeiterfassung&option=zettel_zu_beleg&zettel_id=$id\">BELEG/RECHNUNG ERSTELLEN</a>";
	#echo "<hr>$link_zettel2beleg";
	}
}
...
$f->text_feld("ï¿½berschrift (KW)", "beschreibung", "", "50", 'beschreibung','');
...
weiterleiten_in_sec("?daten=zeiterfassung&option=zettel_eingabe&zettel_id=$mein_letzerzettel_id", 2);
}
...
/*Prï¿½fen ob Leistung in POSITIONEN_KATALOG existiert, falls nicht, hinzufï¿½gen*/
...
weiterleiten("?daten=zeiterfassung&option=zettel_eingabe&zettel_id=$zettel_id");
}
...
#weiterleiten_in_sec("?daten=zeiterfassung&option=zettel_eingabe&zettel_id=$zettel_id", 1);
}
...
header("Content-type: application/pdf");  // wird von MSIE ignoriert
	include_once('pdfclass/class.ezpdf.php');
		...
		$pdf->addText(170,42,6,"BERLUS HAUSVERWALTUNG ï¿½  Fontanestr. 1 ï¿½ 14193 Berlin ï¿½ Inhaber Wolfgang Wehrheim");
		$pdf->addText(150,35,6,"Bankverbindung: Dresdner Bank Berlin ï¿½ BLZ: 100  800  00 ï¿½ Konto-Nr.: 05 804 000 00 ï¿½ Steuernummer: 24/582/61188");
	*/
	...
	$pdf->ezText("$zeile.",10, array('left'=>'10'));
	$pdf->ezSetDy(11); //zeile zurï¿½ck
	$pdf->ezText("$datum",10, array('left'=>'25'));
	$pdf->ezSetDy(11); //zeile zurï¿½ck
	$pdf->ezText("$kostentraeger_typ",10, array('left'=>'75'));
	$pdf->ezSetDy(11); //zeile zurï¿½ck
	$pdf->ezText("$kostentraeger_bez",10, array('left'=>'115'));
	$pdf->ezSetDy(11); //zeile zurï¿½ck
	$pdf->ezSetCmMargins(1,1,1,4);
	if(!empty($hinweis)){
	$pdf->ezText("$beschreibung\n$hinweis",10, array('left'=>'260'));
	}else{
	$pdf->ezText("$beschreibung",10, array('left'=>'260'));
	}
	$pdf->ezSetCmMargins(1,1,1,1);
	$pdf->ezSetDy(13); //zeile zurï¿½ck
	$pdf->ezText("$dauer_std Std. ($dauer_min Min.)",10, array('left'=>'450'));
	...
	$pdf->ezText("<b>Gesamt: $arbeitsdauer</b>",10, array('left'=>'450'));
	#$pdf->ezSetDy(-11); //abstand
	$pdf->ezText("<b>Soll/W $stunden_woche:00</b>",10, array('left'=>'450'));
	$pdf->ezText("<b>Saldo  $saldo_woche_std:$restsaldo_min Std</b>",10, array('left'=>'450'));
	$pdf->ezSetDy(-30); //abstand
	$pdf->ezText("<b>Gesamt bisher: $this->gesamt_azeit_std</b>",10, array('left'=>'400'));
	$pdf->ezText("<b>Gesamtsoll bisher:  $this->gesamt_soll_stunden</b>",10, array('left'=>'400'));
	...
	$pdf->ezText("<b>Stundenkonto: $stundenkonto_in_std</b>",10, array('left'=>'400'));
	
	
	#echo "Soll $stunden_woche:00<br>";
	/*echo "Arbeitsdauer: $arbeitsdauer<br>";
	
	echo "Saldo $saldo_woche_std Std. und $restsaldo_min Minuten<br>";
	echo "<hr>$this->anzahl_eigene_zettel G1:$this->gesamt_azeit_min = $this->gesamt_azeit_std GSOLL $this->gesamt_soll_stunden Std ";
	$link_zettel2beleg = "<a href=\"?daten=zeiterfassung&option=zettel_zu_beleg&zettel_id=$id\">BELEG/RECHNUNG ERSTELLEN</a>";
	echo "<hr>$link_zettel2beleg";
	*/
	}	
...
function zettel2pdf($id){
	...
	*/
	...
	$pdf->ezTable($table_arr,$cols,"Stundennachweis $jahr",
array('showHeadings'=>1,'showLines'=>'1', 'shaded'=>1,'shadeCol'=>array(0.78, 0.95,1), 'shadeCol2'=>array(0.1, 0.5, 1), 'titleFontSize' => 10, 'fontSize' => 8, 'xPos'=>50,'xOrientation'=>'right',  'width'=>500,'cols'=>array('ZEILE'=>array('justification'=>'right', 'width'=>30))));
	$pdf->ezStream();
	}	
...
#$result = mysql_query ("SELECT benutzer_id, benutzername, BP_PARTNER_ID FROM BENUTZER JOIN BENUTZER_PARTNER ON (benutzer_id=BP_BENUTZER_ID) WHERE AUSTRITT > '$datum_h' OR AUSTRITT ='0000-00-00' GROUP BY benutzer_id  ORDER BY `BP_PARTNER_ID`, GEWERK_ID, benutzername ASC");
#$result = mysql_query ("SELECT benutzer_id, benutzername, BP_PARTNER_ID FROM BENUTZER JOIN BENUTZER_PARTNER ON (benutzer_id=BP_BENUTZER_ID)  GROUP BY benutzer_id  ORDER BY `BP_PARTNER_ID`, GEWERK_ID, benutzername ASC");
if($ex==0){
...
echo "<tr class=\"feldernamen\"><td width=150>Kurzname</td><td width=200>Mieter</td><td width=200>Anschrift</td><td width=100>Lage</td><td width=40>mï¿½</td><td>Details</td></tr>\n";
echo "</table>";
iframe_start();
...
$detail_check = detail_check("EINHEIT", $einheit_id);
		if($detail_check>0){
		$detail_link= "<a class=\"table_links\" href=\"?daten=details&detail_tabelle=EINHEIT&detail_id=$einheit_id\">Details</a>"; 	
		}else{
		$detail_link= "<a class=\"table_links\" href=\"?daten=details&detail_tabelle=EINHEIT&detail_id=$einheit_id\">Neues Detail</a>";	
		}	
...
echo "<table>";
		...
		$link_stundenzettel_ansehen = "<a href=\"?daten=zeiterfassung&option=zettel_ansehen&zettel_id=$this->z_zettel_id\">Eingetragen in $this->z_beschreibung</a>";
...
/*SELECT GEWERKE.BEZEICHNUNG, STUNDENZETTEL.BENUTZER_ID, benutzername, STUNDENSATZ, SUM(DAUER_MIN)/60 AS STD, 25*(SUM(DAUER_MIN)/60) AS LEISTUNG_EUR FROM `STUNDENZETTEL_POS` JOIN STUNDENZETTEL ON (STUNDENZETTEL.ZETTEL_ID=STUNDENZETTEL_POS.ZETTEL_ID) JOIN BENUTZER ON(STUNDENZETTEL.BENUTZER_ID=BENUTZER.benutzer_id) JOIN GEWERKE ON (BENUTZER.GEWERK_ID=GEWERKE.G_ID) WHERE  STUNDENZETTEL.AKTUELL = '1' && STUNDENZETTEL_POS.AKTUELL = '1' && KOSTENTRAEGER_TYP='Einheit' && KOSTENTRAEGER_ID='166' GROUP BY STUNDENZETTEL.BENUTZER_ID ORDER BY STD DESC*/

/*SELECT GEWERKE.BEZEICHNUNG,  SUM(DAUER_MIN)/60 AS STD, 25*(SUM(DAUER_MIN)/60) AS LEISTUNG_EUR FROM `STUNDENZETTEL_POS` JOIN STUNDENZETTEL ON (STUNDENZETTEL.ZETTEL_ID=STUNDENZETTEL_POS.ZETTEL_ID) JOIN BENUTZER ON(STUNDENZETTEL.BENUTZER_ID=BENUTZER.benutzer_id) JOIN GEWERKE ON (BENUTZER.GEWERK_ID=GEWERKE.G_ID) WHERE  STUNDENZETTEL.AKTUELL = '1' && STUNDENZETTEL_POS.AKTUELL = '1' && KOSTENTRAEGER_TYP='Einheit' && KOSTENTRAEGER_ID='166' GROUP BY G_ID ORDER BY STD DESC*/

/*SELECT GEWERKE.BEZEICHNUNG, SUM( DAUER_MIN ) /60 AS STD, 25 * ( SUM( DAUER_MIN ) /60 ) AS LEISTUNG_EUR
FROM `STUNDENZETTEL_POS`
JOIN STUNDENZETTEL ON ( STUNDENZETTEL.ZETTEL_ID = STUNDENZETTEL_POS.ZETTEL_ID )
JOIN BENUTZER ON ( STUNDENZETTEL.BENUTZER_ID = BENUTZER.benutzer_id )
JOIN GEWERKE ON ( BENUTZER.GEWERK_ID = GEWERKE.G_ID )
WHERE STUNDENZETTEL.AKTUELL = '1' && STUNDENZETTEL_POS.AKTUELL = '1' && KOSTENTRAEGER_TYP = 'Einheit' && KOSTENTRAEGER_ID = '166' && G_ID = '8'
GROUP BY G_ID
ORDER BY STD DESC
LIMIT 0 , 30*/
