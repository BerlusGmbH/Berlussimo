 ...
 * 
 * @filesource   $HeadURL: http://192.168.2.52/svn/berlussimo_1/tags/02.11.2010 - Downloadversion 0.27/classes/class_person.php $
 * @version      $Revision: 15 $
...
function finde_kos_typ_id($vorname, $nachname){
	...
	if(is_array($personen_ids_arr)){
		...
		if($treffer['ANZ']>1 && isset($_SESSION['geldkonto_id'])){
...
LIMIT 0 , 30";
	...
	if($numrows)
	 {
	while ($row = mysql_fetch_assoc($resultat)) $personen_arr[] = $row;	
	}
