 ...
 * 
 * @filesource   $HeadURL: http://192.168.2.52/svn/berlussimo_1/tags/02.11.2010 - Downloadversion 0.27/classes/class_mietentwicklung.php $
 * @version      $Revision: 6 $
...
function pdf_mietentwicklung($pdf, $mietvertrag_id){
	...
	while ($row1 = mysql_fetch_assoc($result1)){
		...
		$cols = array('KOSTENKATEGORIE'=>"Bezeichnung", 'ANFANG'=>"VON", 'ENDE'=>"BIS", 'BETRAG'=>"BETRAG", 'MWST_ANTEIL'=>"MWSt");
		$heute_d = date("d.m.Y");
		$pdf->ezTable($pdf_aktuell,$cols,"Miete aktuell - Druckdatum: $heute_d - $mvs->einheit_kurzname - $mvs->personen_name_string</b>",
		...
		if(count($pdf_anstehend)>0){
			$cols = array('KOSTENKATEGORIE'=>"Bezeichnung", 'ANFANG'=>"VON", 'ENDE'=>"BIS", 'BETRAG'=>"BETRAG", 'MWST_ANTEIL'=>"MWSt");
			$heute_d = date("d.m.Y");
			$pdf->ezTable($pdf_anstehend,$cols,"Anstehende Änderungen der Miete - Druckdatum: $heute_d - $mvs->einheit_kurzname - $mvs->personen_name_string</b>",
					array('showHeadings'=>1,'shaded'=>1, 'titleFontSize' => 8, 'fontSize' => 8, 'xPos'=>50,'xOrientation'=>'right',  'width'=>500,'cols'=>array('BEZ'=>array('justification'=>'left', 'width'=>140))));
			$pdf->ezSetDy(-5); //abstand
		}
		...
		$temp_kat='';
			...
			$z=1;
				...
				if(count($pdf_abgelaufen)>0){
					$cols = array('KOSTENKATEGORIE'=>"Bezeichnung", 'ANFANG'=>"VON", 'ENDE'=>"BIS", 'BETRAG'=>"BETRAG", 'MWST_ANTEIL'=>"MWSt");
					$heute_d = date("d.m.Y");
					$pdf->ezTable($pdf_abgelaufen,$cols,"Abgelaufene Mietdefinitionen - Druckdatum: $heute_d - $mvs->einheit_kurzname - $mvs->personen_name_string</b>",
							array('showHeadings'=>1,'shaded'=>1, 'titleFontSize' => 8, 'fontSize' => 8, 'xPos'=>50,'xOrientation'=>'right',  'width'=>500,'cols'=>array('BEZ'=>array('justification'=>'left', 'width'=>140))));
				}
