 ...
 * 
 * @filesource   $HeadURL: http://192.168.2.52/svn/berlussimo_1/tags/02.11.2010 - Downloadversion 0.27/classes/class_wartungen.php $
 * @version      $Revision: 6 $
...
$result = mysql_query ("SELECT WARTUNGSPLAN.`PLAN_ID`, IM_EINSATZ, `PLAN_BEZEICHNUNG`, `INTERVALL_NAME`, `FAKTOR`, W_GERAETE.GERAETE_ID, HERSTELLER, W_GERAETE.BEZEICHNUNG FROM `WARTUNGSPLAN` join (WARTUNG_ZUWEISUNG, W_GERAETE) ON (WARTUNG_ZUWEISUNG.PLAN_ID=WARTUNGSPLAN.PLAN_ID && WARTUNG_ZUWEISUNG.GERAETE_ID=W_GERAETE.GERAETE_ID) WHERE DATE_FORMAT(IM_EINSATZ, '%m') = '01' && W_GERAETE.GERAETE_ID NOT IN (SELECT GERAETE_ID FROM WARTUNGEN WHERE DATE_FORMAT(WARTUNGSDATUM, '%Y') = '2010')");	
	...
	/*Liefer ein Array mit Gäreten die im aktuellen und vorjahr nicht gewartet worden sind*/
...
function ausgabe($gruppen_id, $monate_plus_int, $format='tab'){
	...
	$pdf = new Cezpdf('a4', 'landscape');
			...
			$pdf->ezTable($thermen_arr,$cols,"Thermen",
				array('showHeadings'=>1,'shaded'=>1, 'titleFontSize' => 8, 'fontSize' => 7, 'xPos'=>50,'xOrientation'=>'right',  'width'=>700,'cols'=>array('PART'=>array('justification'=>'left','width'=>150),'KOSTENTRAEGER_BEZ'=>array('justification'=>'left','width'=>75),'KONTO'=>array('justification'=>'right','width'=>30),'AUSZUG'=>array('justification'=>'right','width'=>35),'PLATZ'=>array('justification'=>'left','width'=>50))));
