	...
	function konto_tab_anzeigen($bk_k_id, $konto, $profil_id) {
		...
		if ($numrows) {
			...
			while ( $row = mysql_fetch_assoc ( $result ) ) {
				...
				$bk_k_id = $row [BK_K_ID];
				// kostenkonto id aus den bk_konten
				$anteil = $row [ANTEIL];
...
&& DATE_FORMAT(MIETVERTRAG_VON,'%Y') <= '$jahr' && (DATE_FORMAT(MIETVERTRAG_BIS,'%Y') >='$jahr' OR DATE_FORMAT(MIETVERTRAG_BIS,'%Y') ='0000') && EINHEIT_ID='$einheit_id' ORDER BY MIETVERTRAG_VON ASC";
		...
		for($a = 0; $a < $anzahl_summen; $a ++) {
			...
			for($b = 0; $b < $anzahl_einheiten; $b ++) {
				...
				for($c = 0; $c < $anzahl_einheiten_mvs; $c ++) {
					...
					$bk_res [$einheit_name . ' ' . $zeitraum] [] = array (
							'KOSTENART' => "$k->konto_bezeichnung",
							'G_KOS_BEZ' => "$g_kos_bez",
							'G_HNDL' => "$hndl_betrag_anteil",
							'BK_K_ID' => "$bk_k_id",
							'G_BETRAG' => "$summe_konto_a",
							'ANTEIL' => "$anteil_a",
							'UMLAGE' => "$anteil_betrag_a",
							'G_KEY' => "$gesamt_qm m²",
							'QM' => "$einheit_qm",
							'BET_G' => "$beteiligung_a",
							'BET_HNDL' => "$beteiligung_hndl_a",
	...
	function pdf_einzel_tab(&$pdf, $bk_arr, $label, $kontroll_tab_druck) {
		...
		Daraus ergibt sich für Sie folgendes Ergebnis:", 10, array (
			...
			$pdf->ezText ( "Sollte uns keine Einzugsermächtigung vorliegen, bitten wir das Guthaben mit der nächsten Mietzahlung zu verrechnen bzw. den Nachzahlungsbetrag unter Angabe Ihrer Mieternummer $mieternummer auf das <b>Konto $g->kontonummer ( IBAN: $g->IBAN1) bei der $g->kredit_institut, BLZ $g->blz ( BIC: $g->BIC )</b> zu Überweisen.", 10, array (
					'justification' => 'full' 
			) );
			$pdf->ezSetDy ( - 10 );
	...
	function pdf_einzel_tab_160715_falsch(&$pdf, $bk_arr, $label, $kontroll_tab_druck) {
		...
		if ($empf_kos_typ == 'MIETVERTRAG') {
			...
			if ($empf_kos_typ != 'Leerstand') {
				// ##########NEU ENERGIEVERBRAUCH GEGEN VORSCH�SSE###################
				/* pr�fen ob HK Vorsch�sse vorhanden */
				$mz2 = new miete ();
				$met = new mietentwicklung ();
				// $summe_nebenkosten_jahr = $mz->summe_nebenkosten_im_jahr($empf_kos_typ,$empf_kos_id,$this->bk_jahr);
				$summe_hk_vorschuss = $mz2->summe_heizkosten_im_jahr ( $empf_kos_typ, $empf_kos_id, $this->bk_jahr );
			...
			if ($mv->mietvertrag_aktuell) {
				...
				$pdf->ezSetDy ( - 15 );
				$pdf->ezText ( "Die Anpassung des Heiz- und Betriebskostenvorschusses hat eine vertragsverändernde Wirkung, bedarf aber nicht Ihrer Zustimmung. Sollte Sie bei Ihrer Bank einen Dauerauftrag eingerichtet haben, bitten wir diesen ändern zu lassen. Bei uns vorliegenden Einzugserm�chtigung erfolgt automatisch ab $this->bk_verrechnungs_datum_d der Lastschrifteinzug der ge�nderten Miete. \n\n", 10, array (
						'justification' => 'full' 
				) );
				$pdf->ezSetDy ( - 15 );
				// $pdf->ezText("$this->footer_zahlungshinweis",10);
...
Daraus ergibt sich für Sie folgendes Ergebnis:", 10, array (
	...
	function pdf_einzel_tab_1_OK100pro(&$pdf, $bk_arr, $label, $kontroll_tab_druck) {
		...
		$cols1 = array (
				'KOSTENART' => "Betriebskostenart",
				"KOSTEN_GESAMT" => "Kosten gesamt",
				'KOSTEN_GEWERBE' => "Gewerbeanteil $QM_G_GEWERBE m²",
				'KOSTEN_WOHNRAUM' => "Wohnanteil $QM_G m²" 
		);
		// $i=$pdf->ezStartPageNumbers(545,728,6,'','Seite {PAGENUM} von {TOTALPAGENUM}',1);
		$p = new partners ();
		$p->get_partner_info ( $_SESSION [partner_id] );
			...
			if (isset ( $this->bk_an_dat )) {
				...
				if ($mv->mietvertrag_aktuell) {
					...
					$cols = array (
							...
							"AKTUELL" => "Derzeitige Miete",
							'ANPASSUNG' => "Anpassungsbetrag",
							'NEU' => "Neue Miete ab $anp_datum" 
					);
					$pdf->ezTable ( $anp_tab, $cols, "Anpassungstabelle", array (
							'showHeadings' => 1,
							'shaded' => 1,
							'titleFontSize' => 7,
							'fontSize' => 7,
...
Daraus ergibt sich für Sie folgendes Ergebnis:", 10 );
			...
			$pdf->ezSetDy ( - 12 );
			$pdf->ezText ( "Wolfgang Wehrheim", 10 );
			$pdf->ezSetDy ( - 35 );
