	...
	function liste_mietspiegel(){
		...
		if(!is_array($arr)){
			...
			for($a=0;$a<$anz;$a++){
				...
				$ort = $arr[$a]['ORT'];
				if(empty($ort)){
				$link_anzeigen = "<a href=\"?daten=mietspiegel&option=mietspiegel_anzeigen&jahr=$jahr\">$jahr - Ohne Ortsangabe</a><br>";
				}else{
				$link_anzeigen = "<a href=\"?daten=mietspiegel&option=mietspiegel_anzeigen&jahr=$jahr&ort=$ort\">$jahr - $ort</a><br>";
				}	
			echo $link_anzeigen;
			}
