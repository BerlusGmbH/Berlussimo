...
class todo{
	...
	$p = new partners();
	$p->partner_dropdown('Rechnungsempfï¿½nger wï¿½hlen', 'p_id', 'p_id');
...
function baustellen_liste($aktiv=1){
	...
	if(is_array($arr)){
		...
		echo "<thead><tr><th>BAUSTELLE</th><th>RECHNUNGSEMPFï¿½NGER</th><th>OPTIONEN</th></tr></thead>";
		for($a=0;$a<$anz;$a++){
		$bau_id = $arr[$a]['ID'];
		$bez =  $arr[$a]['BEZ'];
...
function form_neue_aufgabe($t_id = NULL, $typ='Benutzer'){
    ...
    $bb->dropdown_kostentreager_typen('Kostentrï¿½gertyp', 'kostentraeger_typ', 'kostentraeger_typ', $js_typ);
    $js_id = "";
    $bb->dropdown_kostentreager_ids('Kostentrï¿½ger', 'kostentraeger_id', 'dd_kostentraeger_id', $js_id);
	}
    #}else{
	...
	if($typ=='Partner'){
	$pp = new partners();
	$pp->partner_dropdown('Partner wï¿½hlen', 'benutzer_id', 'benutzer_id');
	}
...
echo "<channel>\n";
  ...
  $my_proj_id_arr = $this->get_my_projekt_arr($benutzer_id);
	...
	#die("Keine Projekte und Aufgaben fï¿½r Sie vohanden!");
	echo "<title>Keine Projekte und Aufgaben fï¿½r Sie!</title>\n";
	...
	<form>
	<input type="button"  width="60" value=" Lï¿½SCHEN " onclick="mee();"/>	
...
function rss_feed_OK($benutzer_id){
	...
	echo "<link>http://berlussimo-1/berlussimo_workspace/sivac/berlussimo/rss.php</link>\n";
		...
		echo "<link>http://berlussimo-1/berlussimo_workspace/sivac/berlussimo/rss.php</link>\n";
		echo "<description>Angezeigt werden Projekte und dazugehï¿½rige Aufgaben!</description>\n";
		$anz_p = count($my_proj_id_arr);
...
if($anz){
	...
	$f = new formular();
	#ob_clean();
	$f->fieldset("Meine Auftrï¿½ge", 'ana');
...
function form_suche($typ_int_ext='intern'){
	...
	if($typ_int_ext=='intern'){
		$be = new benutzer;
		$f->check_box_js('check_int_ext', 1, 'Externe Auftrï¿½ge suchen', 'onchange="redirect_to(\'?daten=todo&option=auftrag_suche&typ_int_ext=extern\')"', null);
		$be->dropdown_benutzer2('Mitarbeiter wï¿½hlen', 'benutzer_id', 'benutzer_id', null);
		$f->hidden_feld('benutzer_typ', 'benutzer');
	}else{
		$p = new partner;
		$f->check_box_js('check_int_ext', 1, 'Interne Auftrï¿½ge suchen', 'onchange="redirect_to(\'?daten=todo&option=auftrag_suche&typ_int_ext=intern\')"', null);
		echo "<br>";
		$p->partner_dropdown('Externe Firma/Partner wï¿½hlen', 'benutzer_id', 'benutzer_id', null);
		$f->hidden_feld('benutzer_typ', 'Partner');
		
	}
	$f->hidden_feld('option', 'auftrag_suche_send');
	$f-> send_button('BTN_SuchA', 'Auftrï¿½ge finden');
	$f->ende_formular();
...
function form_edit_aufgabe($t_id){
	if(empty($t_id)){
		die('Aufgabe oder Projekt wï¿½hlen');
	}
	$this->get_aufgabe_alles($t_id);
	#echo '<pre>';
	#print_r($this);
	...
	#echo "<a href=\"?daten=todo&option=projekt_loeschen&t_id=$t_id\">Projekt lï¿½schen ???</a><br>";
	}else{
	$f->fieldset("Aufgabe bearbeiten:$this->text", 'na');	
	#echo "<a href=\"?daten=todo&option=projekt_loeschen&t_id=$t_id\">Aufgabe lï¿½schen ???</a><br>";
	}
	...
	$js_typ = "onchange=\"list_kostentraeger('list_kostentraeger', this.value)\"";
    #$bb->dropdown_kostentreager_typen('Kostentrï¿½gertyp', 'kostentraeger_typ', 'kostentraeger_typ', $js_typ);
    $bb->dropdown_kostentreager_typen_vw('Kostentrï¿½gertyp', 'kostentraeger_typ', 'kostentraeger_typ', $js_typ, $this->kos_typ);
    $js_id = "";
    #$bb->dropdown_kostentreager_ids('Kostentrï¿½ger', 'kostentraeger_id', 'dd_kostentraeger_id', $js_id);
    $bb->dropdown_kostentraeger_bez_vw('Kostentrï¿½ger', 'kostentraeger_id', 'dd_kostentraeger_id', $js_id, $this->kos_typ, $this->kos_id);
	...
	$pp = new partners();
	$pp->partner_dropdown('Partner wï¿½hlen', 'benutzer_id', 'benutzer_id', $this->benutzer_id);
	}
	...
	$f->send_button('submit_n1', 'ï¿½nderungen speichern');
	$f->fieldset_ende();
	$f->ende_formular();
...
function projekt_aufgabe_loeschen($t_id){
	$this->get_aufgabe_alles($t_id);
	$f = new formular();
	$f->erstelle_formular('Lï¿½schen von Projekten und Aufgaben','');
	$f->fieldset('Lï¿½schen', 'loeschen');
	if($this->ue_id=='0'){
		echo "Ganzes Projekt <b>$this->text</b> lï¿½schen?<br><br>";
	$f->hidden_feld('art', 'Projekt');
	}else{
		echo "Aufgabe: <b>$this->text</b> lï¿½schen?<br><br>";
...
$cols = array('POS'=>"POS",'T_ID'=>"ANR", 'TEXT'=>"Aufgaben", 'KOS_BEZ'=>"Ort", 'ERLEDIGT_STAT'=>"Status", 'MITARBEITER'=>"Mitarbeiter");

$pdf->ezText("$projekt_name",11);
$pdf->ezTable($arr,$cols,"<b>Projekt: $id</b>",array('showHeadings'=>1,'shaded'=>1, 'titleFontSize' => 7, 'fontSize' => 7, 'xPos'=>55,'xOrientation'=>'right',  'width'=>500,'cols'=>array('SEITE'=>array('justification'=>'left','width'=>27),'EINHEIT'=>array('justification'=>'left','width'=>50),'ZEITRAUM'=>array('justification'=>'left','width'=>90),'EMPF'=>array('justification'=>'left'))));
...
function auftraege_an_haus($haus_id){
	...
	if(!is_array($arr)){
		...
		fehlermeldung_ausgeben("Keine Auftrï¿½ge an Haus $h->haus_strasse $h->haus_nummer");
	}else{
