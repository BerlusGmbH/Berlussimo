(function ($) {
    ...
    $.fn.materialize_chips_autocomplete = function (options) {
        var self = this;
        this.$el = $(this);
        ...
        this.handleEvents = function () {
            ...
            self.$document.on('keydown', SELS.CHIPS + ' ' + SELS.INPUT, function (e) {
                ...
                var $autocomplete = $target.siblings('.autocomplete-content .dropdown-content').first();
                var value = $autocomplete.find('li').first().text().trim();

                // enter
                if (13 === e.which) {
                    e.preventDefault();
                    self.addChip(chipsIndex, {tag: value}, $chips);
                    $target.val('');
                    return;
                }
        ...
        this.chips = function ($chips) {
            ...
            html += '<input class="input" type="text" placeholder="">';
            $chips.html(html);
        ...
        if (!$.isEmptyObject(data)) {
                ...
                if (val !== '') {
                    for (var key in data) {
                        if (data.hasOwnProperty(key) &&
                            ...
                            if (!!data[key].img) {
                                autocompleteOption.append('<img src="' + data[key].img + '" class="right circle"><span>' + key + '</span>').data('id', data[key].id);
                            } else {
                                autocompleteOption.append('<span>' + key + '</span>').data('id', data[key].id);
                            }
            ...
            $autocomplete.on('click', 'li', function (e) {
                ...
                //var chipsLength = $chips.children(SELS.CHIP).length;
                self.addChip(chipsIndex, {tag: value, id: id}, $chips);
                e.preventDefault();
                $input.val('');
                $input.focus();
                $autocomplete.empty();
