<div id="{{$id}}" class="modal">
    <form action="{{route('web::persons.jobs.store', ['person' => $person])}}" method="post">
        <div class="modal-content">
            <h4>Arbeitnehmer</h4>
            <div class="row">
                <div class="input-field col s12">
                    <i class="mdi mdi-account-multiple prefix"></i>
                    <input type="text" id="{{$id}}_employer" name="employer" value="{{old('employer')}}"
                           class="autocomplete validate {{$errors->has('employer') ? 'invalid' : ''}}"
                           autocomplete="off">
                    <span class="error-block">{{$errors->has('employer') ? $errors->first('employer') : ''}}</span>
                    <label for="{{$id}}_employer">Arbeitgeber</label>
                </div>
                <div class="input-field col s12">
                    <i class="mdi mdi-book-open-page-variant prefix"></i>
                    <input type="text" id="{{$id}}_title" name="title" value="{{old('title')}}"
...
<script type="text/javascript">
    $('document').ready(function () {
        @if($errors->has('employer'))
            $('#{{$id}}').modal('open');
        @endif
        $('#{{$id}}_employer').autocomplete({
            data: {
        @foreach(\App\Models\Partner::defaultOrder()->get(['PARTNER_NAME']) as $partner)
        {!! json_encode(trim($partner->PARTNER_NAME)) !!} :
        null,
        @endforeach
    }
    })
