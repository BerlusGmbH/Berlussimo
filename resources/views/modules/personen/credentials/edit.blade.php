<div id="{{$id}}" class="modal">
    <form action="{{route('web::persons.credentials.update', ['person' => $person])}}" method="post">
        {{method_field('PATCH')}}
        <div class="modal-content">
            ...
            <div class="row">
                ...
                <div class="input-field col-xs-2">
                    <input class="filled-in" type="checkbox" id="inactive"
                           name="inactive" {{$person->credential->trashed() ? 'checked' : ''}}>
                    <label for="inactive">Deaktiviert</label>
                </div>
            </div>
            <h5>Rollen</h5>
            <div class="row">
                @foreach(\Spatie\Permission\Models\Role::all() as $key => $role)
                    <div class="input-field col-xs-12 col-sm-6 col-md-4 col-lg-3">
                        <input class="filled-in" type="checkbox" id="role_{{$key}}" name="roles[]"
                               value="{{$role->name}}" {{in_array($role->name, old('roles', $person->roles->pluck('name')->all())) ? 'checked' : ''}}>
