...
class BarGraph {
	...
	public function BarGraph() {
		...
		$this->stylesheetUrl = $this->SVGraphRoot . 'css/BarGraph.css';
		$this->ecmascriptUrl = $this->SVGraphRoot . 'js/BarGraph.js';
	...
	private function outputSVGStart() {
			...
			private function outputGraphObjects() {
				...
				for($i=0; $i<$this->nrOfDataRows; $i++) {
					...
					for($j=0; $j<count($this->dataRow[$i]); $j++) {
						...
						// calculate coordinates/values for this MP
						$value = $this->dataRow[$i][$j];
						$width = $this->rectWidth;
						$height = $value/($this->maxValue - $this->minValue) * 100;
							if($height < 0) $height *= -1;
						$x = $this->rectMargin/2 + $this->rectWidth*$i + $this->measuringPointWidth*$j;
						if($value >= 0)
							$y = ($this->maxValue - $value) / ($this->maxValue - $this->minValue) * 100;
						else
							$y = $this->nullPoint;
		...
		private function outputLegend() {
			...
			for($i=0; $i<$this->nrOfDataRows; $i++) {
				...
				echo "\t" . '<rect id="legendDataRow'.$i.'" class="dataRow'.$i.'" width="30" height="20" />' . "\n";
			}
			// averages
			foreach($this->averages as $key => $value) {
				if($key === 'total') {
