 ...
 * 
 * @filesource   $HeadURL: http://192.168.2.52/svn/berlussimo_1/tags/02.11.2010 - Downloadversion 0.27/options/modules/mietkonten_blatt.php $
 * @version      $Revision: 6 $
...
if (! isset ( $_SESSION ['benutzer_id'] ) or ! check_user_mod ( $_SESSION ['benutzer_id'], 'mietkonten_blatt' )) {
	...
	if (isset ( $anzeigen )) {
		switch ($anzeigen) {
			...
			case "mk_pdf" :
				...
				if (! empty ( $mv_id )) {
					$mz->mietkonten_blatt_pdf ( $mv_id ); // ALT
			...
			case "mietkonto_uebersicht_detailiert" :
				 ...
				 * echo "<tr><td>Datum</td><td>Monatssoll</td><td>Zahlung</td><td><b>Saldo</b></td></tr>";
				 * if(!empty($a->saldo_vv)){
				 * echo "<tr><td colspan=3 align=left><b>SALDO VORTRAG VORVERWALTUNG</td><td><b>$a->saldo_vv �</b></td></tr>";
				 * }
				 * foreach($a->daten_arr as $key=>$value){
				 * for($b=0;$b<count($a->daten_arr[$key][monate]);$b++){
				 * //Miete Sollzeile
				 * echo "<tr><td>1.".$a->daten_arr[$key][monate][$b][monat].".$key</td><td>Soll Miete ".$a->daten_arr[$key][monate][$b][monat].".$key </td><td><b>".$a->daten_arr[$key][monate][$b][soll]." �</b></td><td></td></tr>";
				 *
				 * /*Zeilen Zahlungen
				 */
				/*
				 ...
				 * echo "<tr><td>$datum</td><td>".$a->daten_arr[$key][monate][$b][zahlungen][$c][BEMERKUNG]."</td><td>".$a->daten_arr[$key][monate][$b][zahlungen][$c][BETRAG]."</td><td></td></tr>\n";
				 * }
			...
			case "mietkonto_uebersicht_detailiert_ALT" :
				...
				$zeitraum = new zeitraum (); // Zeitraum Klasse f�r den Monatearray
				...
				// ##########ENDE DER VORBEREITUNG DER NOTWENDIGEN DATEN F�R DIE BERECHUNG##################################################
				...
				for($i = 0; $i < count ( $monate_arr ); $i ++) {
					...
					// zahlungen aus aktuellen monat z�hlen
					...
					// ######################### ENDE ANDERE MONATE##########################################
				} // ende f�r (for $i) alle monate durchlaufen
				  // ###########################################################
				  // tabellenende
			...
			case "mietkonto_detailiert_seit_1zahlung" :
				...
				echo "</p>";
				echo "<p><a href=\"mietkonto_pdf.php?mietvertrag_id=$mietvertrag_id\"><img src=\"pdfclass/pdf_logo.gif\" width=30 ></a></p>";
				echo "<hr>";
				// ###########################################
				// $buchung->array_anzeigen($mieter_daten_arr);
				// ###########################################
			...
			case "forderung_aus_monat" :
				iframe_start ();
				echo "Forderung aus Monat $_REQUEST[monat] $_REQUEST[jahr]";
				$buchung = new mietkonto ();
				$forderung_arr = $buchung->forderung_monatlich ( $mietvertrag_id, $_REQUEST [monat], $_REQUEST [jahr] );
				$monat_jahr = "" . $_REQUEST [monat] . "" . $_REQUEST [jahr] . "";
			...
			case "miete_manuell_buchen" :
				...
				if (isset ( $_REQUEST ['mietvertrag_id'] ) && ! empty ( $_REQUEST ['mietvertrag_id'] )) {
					$mietvertrag_id = $_REQUEST ['mietvertrag_id'];
					echo "BUCHUNGS_FORM F�R $mietvertrag_id";
					$buchung = new mietkonto ();
	...
	if ($anzahl_zahlungen_im_monat > 1) {
		...
		if ($gesamt_zahlung_und_konto >= $summe_forderungen) {
			...
			if (($rest) > $heizkosten) {
				...
				// echo "<hr><hr>�BERSCHUSS = $ueberschuss �";
			}
		}
	}
	echo "</td><td>"; // Zelle 4
