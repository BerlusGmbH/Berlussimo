...
switch($mietvertrag_raus) {
    ...
    */
    ...
    $mv_info = new mietvertraege;
    $mv_info->teilnahme_einzugsverfahren_eingeben($_POST[mietvertrag_id], $_POST[konto_inhaber_autoeinzug], $_POST[konto_nummer_autoeinzug], $_POST[blz_autoeinzug], $_POST[geld_institut], $_POST[einzugsart], 'JA');    weiterleiten_in_sec("?daten=mietvertrag_raus&mietvertrag_raus=ls_teilnehmer", 2);
    break;
    ...
    */
    ...
    if(!empty($_POST[miete_kalt])){
    	...
    	#echo "Kaltmiete betrag nicht ok";
    	$error .= 'Kaltmiete Betrag fehlerhaft<br>';
    }else{
    	#echo "Kaltmiete betrag ok";
    	}
    }else{
...
weiterleiten_in_sec("?daten=uebersicht&anzeigen=einheit&einheit_id=$_POST[einheit_id]", "1");	
   iframe_end();
	...
	#echo "<h1>$_POST[mietvertrag_id] $mietvertrag_bis</h1>";
	...
	#hinweis_ausgeben("Teilnahme am Lastschriftverfahren wurde beendet");    
...
fehlermeldung_ausgeben("Mietvertrag zum Ändern auswählen");     weiterleiten_in_sec('?daten=mietvertrag_raus&mietvertrag_raus=mietvertrag_kurz"', '2');
    }
    $form->ende_formular();
    break;
...
while (list ($MIETVERTRAG_ID, $MIETVERTRAG_VON, $EINHEIT_ID) = mysql_fetch_row($resultat1)){
	...
	$resultat2 = mysql_query($db_abfrage2) or
           die(mysql_error()); // Neuer Datensatz mit aktuellem Datum
...
$resultat3 = mysql_query($db_abfrage3) or
           die(mysql_error());
...
$counter = 0;
		...
		if($counter == 1){
	    echo "<tr class=\"zeile1\"><td width=100>$einheit_link $mietkonto_link $miete_aendern</td><td width=300>($mieter_im_vetrag)";echo mieterid_zum_vertrag($MIETVERTRAG_ID);echo "</td><td width=80>$MIETVERTRAG_VON</td><td width=80>$MIETVERTRAG_BIS</td><td>$detail_link</td><td>$beenden_link $aendern_link $mv_loeschen_link</td></tr>";
		}
		if($counter == 2){
		echo "<tr class=\"zeile2\"><td width=100>$einheit_link $mietkonto_link $miete_aendern</td><td width=300>($mieter_im_vetrag)";echo mieterid_zum_vertrag($MIETVERTRAG_ID);echo "</td><td width=80>$MIETVERTRAG_VON</td><td width=80>$MIETVERTRAG_BIS</td><td>$detail_link</td><td>$beenden_link $aendern_link $mv_loeschen_link</td></tr>";
		$counter = 0;
		} 
	}
echo "</table>";		 
...
if(empty($einheit_id)){
$datum_heute = date("Y-m-d");
#$db_abfrage = "SELECT DISTINCT MIETVERTRAG.MIETVERTRAG_ID, MIETVERTRAG.MIETVERTRAG_VON, MIETVERTRAG.MIETVERTRAG_BIS, MIETVERTRAG.EINHEIT_ID, EINHEIT.EINHEIT_KURZNAME FROM MIETVERTRAG, EINHEIT WHERE MIETVERTRAG_AKTUELL='1' && EINHEIT.EINHEIT_AKTUELL='1' && EINHEIT.EINHEIT_ID = MIETVERTRAG.EINHEIT_ID && ((MIETVERTRAG.MIETVERTRAG_BIS!='0000-00-00') && (MIETVERTRAG.MIETVERTRAG_BIS>$datum_heute)) ORDER BY EINHEIT.EINHEIT_KURZNAME ASC";	
...
$counter = 0;
		...
		if($counter == 1){
	    echo "<tr class=\"zeile1\"><td>$einheit_link</td><td>($mieter_im_vetrag)";echo mieterid_zum_vertrag($MIETVERTRAG_ID);echo "</td><td>$mietkonto_link $buchen_link $miete_aendern  $kautionen_link</td><td>$MIETVERTRAG_VON</td><td>$MIETVERTRAG_BIS</td><td>$detail_link</td><td>$beenden_link $aendern_link</td></tr>";
		}
		if($counter == 2){
		echo "<tr class=\"zeile2\"><td>$einheit_link </td><td>($mieter_im_vetrag)";echo mieterid_zum_vertrag($MIETVERTRAG_ID);echo "</td><td>$mietkonto_link $buchen_link $miete_aendern  $kautionen_link</td><td>$MIETVERTRAG_VON</td><td>$MIETVERTRAG_BIS</td><td>$detail_link</td><td>$beenden_link $aendern_link</td></tr>";
		$counter = 0;
		} 
	}
echo "</table>";		 
...
$counter = 0;
		...
		if($counter == 1){
	    echo "<tr class=\"zeile1\"><td>$einheit_link $miete_aendern $kautionen_link </td><td>($mieter_im_vetrag)";echo mieterid_zum_vertrag($MIETVERTRAG_ID);echo "</td><td>$MIETVERTRAG_VON</td><td>$MIETVERTRAG_BIS</td><td>$detail_link</td><td>$beenden_link $aendern_link</td></tr>";
		}
		if($counter == 2){
		echo "<tr class=\"zeile2\"><td>$einheit_link $miete_aendern $kautionen_link</td><td>($mieter_im_vetrag)";echo mieterid_zum_vertrag($MIETVERTRAG_ID);echo "</td><td>$MIETVERTRAG_VON</td><td>$MIETVERTRAG_BIS</td><td>$detail_link</td><td>$beenden_link $aendern_link</td></tr>";
		$counter = 0;
		} 
	}
echo "</table>";		 
