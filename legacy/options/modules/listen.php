...
switch ($option) {
    ...
    case "upload_auszug" :
        ...
        if (request()->hasFile('file')) {
            if (session()->has('kto_auszug_arr')) {
                session()->forget('kto_auszug_arr');
            }

            $datei = request()->file('file')->getRealPath();
            $li = new listen ();
            session()->put('kto_auszug_arr', $li->parse_auszug($datei));
        }
        if (is_array(session()->get('kto_auszug_arr'))) {

            if (request()->has('ds_id')) {
                session()->put('kto_auszug_ds', request()->input('ds_id'));
            }

            if (!session()->has('kto_auszug_ds')) {
                session()->put('kto_auszug_ds', 0);
            }

            if (request()->has('next')) {
                session()->put('kto_auszug_ds', session()->get('kto_auszug_ds') + 1);
            }
            ...
            $anz_ok = count(session()->get('kto_auszug_arr')) - 2;
            if (session()->get('kto_auszug_ds') >= $anz_ok or session()->get('kto_auszug_ds') < 0) {
                session()->put('kto_auszug_ds', 0);
            }

            $sep->form_ds_kontoauszug(session()->get('kto_auszug_ds'));
            if (session()->has('geldkonto_id') && session()->has('temp_kontoauszugsnummer')) {
                $bu = new buchen ();
    ...
    case "excel_einzelbuchung" :
        ...
        $kto_auszugsnr = request()->input('temp_kontoauszugsnummer');
