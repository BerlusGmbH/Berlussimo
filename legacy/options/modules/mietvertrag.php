...
switch ($mietvertrag_raus) {
    ...
    case "ls_teilnehmer_aktivieren" :
        ...
        weiterleiten_in_sec(route('legacy::mietvertraege::index', ['mietvertrag_raus' => 'ls_teilnehmer'], false), 1);
        $form->ende_formular();
        break;
    ...
    case "ls_teilnehmer_deaktivieren" :
        ...
        weiterleiten_in_sec(route('legacy::mietvertraege::index', ['mietvertrag_raus' => 'ls_teilnehmer_inaktiv'], false), 1);
        $form->ende_formular();
        break;
    ...
    case "ls_neu_speichern" :
        ...
        weiterleiten_in_sec(route('legacy::mietvertraege::index', ['mietvertrag_raus' => 'ls_teilnehmer'], false), 2);
        break;
    ...
    case "ls_bearbeitet_speichern";
        ...
        weiterleiten_in_sec(route('legacy::mietvertraege::index', ['mietvertrag_raus' => 'ls_teilnehmer_neu', 'mietvertrag_id' => request()->input('mietvertrag_id')], false), 2);
        break;
    ...
    case "mv_speichern" :
        ...
        weiterleiten_in_sec(route('legacy::uebersicht::index', ['anzeigen' => 'einheit', 'einheit_id' => request()->input('einheit_id')], false), "1");
        iframe_end();
        $form->ende_formular();
        break;
    ...
    case "mahnliste_alle" :
        ...
        if (session()->has('objekt_id')) {
            ...
            if (!request()->has('pdf')) {
                $obj_id = session()->get('objekt_id');
                $link_pdf = "<a href='" . route('legacy::mietvertraege::index', ['mietvertrag_raus' => 'mahnliste_alle', 'objekt_id' => $obj_id, 'pdf'], false) . "'>Als PDF anzeigen</a>";
                echo $link_pdf;
                $ma->finde_schuldner('');
            } else {
    ...
    case "mahnliste_ausgezogene" :
        ...
        if (session()->has('objekt_id')) {
            ...
            if (!request()->has('pdf')) {
                $obj_id = session()->get('objekt_id');
                $link_pdf = "<a href='" . route('legacy::mietvertraege::index', ['mietvertrag_raus' => 'mahnliste_ausgezogene', 'objekt_id' => $obj_id, 'pdf']) . "'>Als PDF anzeigen</a>";
                echo $link_pdf;
    ...
    case "letzte_auszuege" :
        $f = new formular ();
        $link = route('legacy::mietvertraege::index', ['mietvertrag_raus' => 'letzte_auszuege'], false);
    ...
    case "letzte_einzuege" :
        $f = new formular ();
        $link = route('legacy::mietvertraege::index', ['mietvertrag_raus' => 'letzte_einzuege'], false);
        $b = new berlussimo_global ();
        $b->objekt_auswahl_liste();
        $m = new mietvertraege ();
        $objekt_id = session()->get('objekt_id');
    ...
    case "alle_letzten_auszuege" :
        $f = new formular ();
        $b = new berlussimo_global ();
        $link = route('legacy::mietvertraege::index', ['mietvertrag_raus' => 'alle_letzten_auszuege'], false);
        $m = new mietvertraege ();
        $jahr = request()->input('jahr');
        $monat = request()->input('monat');
    ...
    case "alle_letzten_einzuege" :
        $f = new formular ();
        $b = new berlussimo_global ();
        $link = route('legacy::mietvertraege::index', ['mietvertrag_raus' => 'alle_letzten_einzuege'], false);
        $m = new mietvertraege ();
        $jahr = request()->input('jahr');
        $monat = request()->input('monat');
    ...
    case "nebenkosten" :
        ...
        $bg = new berlussimo_global ();
        $link = route('legacy::mietvertraege::index', ['mietvertrag_raus' => 'nebenkosten'], false);
        $bg->objekt_auswahl_liste();
...
{
    if (request()->has('submit_mv_beenden')) {
    ...
    if (request()->exists('submit_mv_beenden')) {
        if (empty (request()->input('MIETVERTRAG_BIS'))) {
            echo "datum eingeben";
        } else {
            mietvertrag_beenden(request()->input('MIETVERTRAG_DAT'), request()->input('MIETVERTRAG_BIS'));
            $einheit_name = einheit_kurzname(request()->input('EINHEIT_ID'));
            hinweis_ausgeben("Mietvertrag für die Einheit $einheit_name wird zum " . request()->input('MIETVERTRAG_BIS') . " beendet!");
        }
    }
}
...
function mietvertrag_aendern_form($mietvertrag_id)
{
    ...
    if (request()->exists('submit_mv_pruefen')) {
        ...
        weiterleiten(route('legacy::uebersicht::index', ['anzeigen' => 'einheit', 'einheit_id' => request()->input('EINHEIT_ID')], false));
    }
}
...
function mietvertrag_beenden($mietvertrag_dat, $mietvertrag_bis)
{
    ...
    weiterleiten(route('legacy::uebersicht::index', ['anzeigen' => 'einheit', 'einheit_id' => $akt_einheit_id], false));
}
...
function mietvertrag_kurz($einheit_id)
{
    ...
    if ($numrows < 1) {
        ...
        while (list ($MIETVERTRAG_ID, $MIETVERTRAG_VON, $MIETVERTRAG_BIS, $EINHEIT_ID) = mysql_fetch_row($resultat)) {
            ...
            $mietkonto_link = "<a href='" . route('legacy::mietkontenblatt::index', ['anzeigen' => 'mietkonto_uebersicht_detailiert', 'mietvertrag_id' => $MIETVERTRAG_ID]) . "'>MIETKONTO</a>";
            $miete_aendern = "<a href='" . route('legacy::miete_definieren::index', ['option' => 'miethoehe', 'mietvertrag_id' => $MIETVERTRAG_ID]) . "'>MIETHÖHE</a>";
            $einheit_link = "<a href='" . route('legacy::uebersicht::index', ['anzeigen' => 'einheit', 'einheit_id' => $EINHEIT_ID]) . "'>$einheit_kurzname</a>";
            $mv_loeschen_link = "<a href='" . route('legacy::mietvertraege::index', ['mietvertrag_raus' => 'mv_loeschen', 'mv_id' => $MIETVERTRAG_ID]) . "'>MV löschen</a>";

            if ($detail_check > 0) {
                $detail_link = "<a class=\"table_links\" href='" . route('legacy::details::index', ['option' => 'details_anzeigen', 'detail_tabelle' => 'MIETVERTRAG', 'detail_id' => $MIETVERTRAG_ID]) . "'>Details</a>";
            } else {
                $detail_link = "<a class=\"table_links\" href='" . route('legacy::details::index', ['option' => 'details_hinzu', 'detail_tabelle' => 'MIETVERTRAG', 'detail_id' => $MIETVERTRAG_ID]) . "'>Neues Detail</a>";
            }
            if ($counter == 1) {
...
function mietvertrag_abgelaufen($einheit_id)
{
    ...
    if ($numrows < 1) {
        ...
        while (list ($MIETVERTRAG_ID, $MIETVERTRAG_VON, $MIETVERTRAG_BIS, $EINHEIT_ID) = mysql_fetch_row($resultat)) {
            ...
            $datum_heute = date("Y-m-d");
            if (($MIETVERTRAG_BIS > $datum_heute) or ($MIETVERTRAG_BIS == "0000-00-00")) {
                $beenden_link = "<a href='" . route('legacy::mietvertraege::index', ['mietvertrag_raus' => 'mietvertrag_beenden', 'einheit_id' => $EINHEIT_ID]) . "'>Beenden</a>";
                $aendern_link = "<a href='" . route('legacy::mietvertraege::index', ['mietvertrag_raus' => 'mietvertrag_aendern', 'einheit_id' => $EINHEIT_ID]) . "'>Ändern</a>";
            } else {
                $beenden_link = "Abgelaufen";
                $aendern_link = "k.Ä.";
            }
            ...
            $einheit_kurzname = einheit_kurzname($EINHEIT_ID);
            $detail_check = detail_check("MIETVERTRAG", $MIETVERTRAG_ID);
            $buchen_link = "<a href='" . route('legacy::miete_buchen::index', ['schritt' => 'buchungsauswahl', 'mietvertrag_id' => $MIETVERTRAG_ID]) . "'>BUCHEN</a>";
            $mietkonto_link = "<a href='" . route('legacy::mietkontenblatt::index', ['anzeigen' => 'mietkonto_uebersicht_detailiert', 'mietvertrag_id' => $MIETVERTRAG_ID]) . "'>MIETKONTO</a>";
            $miete_aendern = "<a href='" . route('legacy::miete_definieren::index', ['option' => 'miethoehe', 'mietvertrag_id' => $MIETVERTRAG_ID]) . "'>MIETHÖHE</a>";
            $einheit_link = "<a href='" . route('legacy::uebersicht::index', ['anzeigen' => 'einheit', 'einheit_id' => $EINHEIT_ID]) . "'>$einheit_kurzname</a>";
            $kautionen_link = "<a href='" . route('legacy::kautionen::index', ['option' => 'kautionen_buchen', 'mietvertrag_id' => $MIETVERTRAG_ID]) . "'>KAUTION BUCHEN</a>";
...
function mietvertrag_objekt_links()
{
    ...
    while (list ($OBJEKT_DAT, $OBJEKT_ID, $OBJEKT_KURZNAME) = mysql_fetch_row($resultat)) {
        echo "<a class=\"objekt_links\" href='" . route('legacy::mietvertraege::index', ['mietvertrag_raus' => 'mietvertrag_neu', 'objekt_id' => $OBJEKT_ID]) . "'>$OBJEKT_KURZNAME</a><br>\n";
    }
}
