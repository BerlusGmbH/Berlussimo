...
class general
{
    ...
    function get_termin_arr($TEAM_ID = 1, $datum_df, $tage = 7)
    {
        ...
        for ($a = 0; $a < $anz; $a++) {
            ...
            $abfrage = "SELECT '$datum_d' AS DATUM, DATE_FORMAT('$datum_sql','%Y%m%d') AS DATUMZ, name, W_TEAMS_BENUTZER.BENUTZER_ID,  TERMINE_TAG,  (SELECT COUNT(DAT) FROM GEO_TERMINE WHERE DATUM='$datum_sql' && W_TEAMS_BENUTZER.BENUTZER_ID=GEO_TERMINE.BENUTZER_ID && AKTUELL='1') AS T_BELEGT, TERMINE_TAG-(SELECT COUNT(DAT) FROM GEO_TERMINE WHERE DATUM='$datum_sql' && W_TEAMS_BENUTZER.BENUTZER_ID=GEO_TERMINE.BENUTZER_ID && AKTUELL='1') AS FREI, START_ADRESSE  FROM users, `W_TEAMS_BENUTZER`, W_TEAM_PROFILE, GEO_TERMINE WHERE W_TEAM_PROFILE.$wochentag_nr='1' AND `TEAM_ID` = '$TEAM_ID' AND W_TEAMS_BENUTZER.AKTUELL = '1' AND W_TEAM_PROFILE.BENUTZER_ID=W_TEAMS_BENUTZER.BENUTZER_ID AND W_TEAM_PROFILE.AKTUELL='1' AND W_TEAM_PROFILE.AKTIV='1' AND TERMINE_TAG>(SELECT COUNT(*) AS ANZ FROM GEO_TERMINE WHERE GEO_TERMINE.BENUTZER_ID=W_TEAMS_BENUTZER.BENUTZER_ID && GEO_TERMINE.AKTUELL='1' && DATUM='$datum_sql') && W_TEAMS_BENUTZER.BENUTZER_ID NOT IN(SELECT BENUTZER_ID FROM URLAUB WHERE URLAUB.DATUM='$datum_sql' && URLAUB.AKTUELL='1') && users.id=W_TEAMS_BENUTZER.BENUTZER_ID  GROUP BY  W_TEAMS_BENUTZER.BENUTZER_ID ORDER BY DATUMZ ASC, FREI ASC";
