...
function get_entfernung($lon, $lat)
{
    if (session()->has('w_datum')) {
        ...
        $result = DB::select("SELECT name, GEO_TERMINE.*, GEO_LON_LAT.*, DATE_FORMAT(GEO_TERMINE.DATUM, '%d.%m.%Y') AS DATUM_G, DATEDIFF(DATUM, '$wunschdatum') AS DIFF,round( acos( sin( $lat * ( pi( ) /180 ) ) * sin( LAT * ( pi( ) /180 ) ) + cos( $lat * ( pi( ) /180 ) ) * cos( LAT * ( pi( ) /180 ) ) * cos( ( $lon * ( pi( ) /180 ) ) - ( LON * ( pi( ) /180 ) ) ) ) *6370, 2 ) AS ENTF_KM, DATEDIFF(DATUM, '$wunschdatum') * round( acos( sin( $lat * ( pi( ) /180 ) ) * sin( LAT * ( pi( ) /180 ) ) + cos( $lat * ( pi( ) /180 ) ) * cos( LAT * ( pi( ) /180 ) ) * cos( ( $lon * ( pi( ) /180 ) ) - ( LON * ( pi( ) /180 ) ) ) ) *6370, 2 ) AS WERTUNG FROM `GEO_TERMINE`, GEO_LON_LAT, users WHERE GEO_LONLAT_ID = GEO_LON_LAT.DAT && DATUM >= '$wunschdatum' && BENUTZER.benutzer_id=GEO_TERMINE.BENUTZER_ID GROUP BY DATUM ORDER BY WERTUNG ASC, `ENTF_KM` ASC, DATUM ASC");
    }
