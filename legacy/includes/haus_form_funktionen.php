...
function objekt_liste_links() {
	...
	while ( list ( $OBJEKT_DAT, $OBJEKT_ID, $OBJEKT_KURZNAME ) = mysql_fetch_row ( $resultat ) ) {
		echo "<a class=\"objekt_links\" href='" . route('legacy::haeuserform::index', ['daten_rein' => 'anlegen', 'haus_objekt' => $OBJEKT_ID]) . "'>$OBJEKT_KURZNAME</a><br>\n";
	}
}
function objekt_liste_links_aenderung() {
	...
	echo "<b>Objekt auswählen:</b><br>\n ";
	while ( list ( $OBJEKT_DAT, $OBJEKT_ID, $OBJEKT_KURZNAME ) = mysql_fetch_row ( $resultat ) ) {
		echo "<a class=\"objekt_links\" href='" . route('legacy::haeuserform::index', ['daten_rein' => 'aendern_liste', 'objekt_id' => $OBJEKT_ID]) . "'>$OBJEKT_KURZNAME</a><br>\n";
	}
}
...
function haeuser_liste_tabellealt($objekt_id) {
	...
	if ($numrows > 0) {
		...
		while ( list ( $HAUS_DAT, $HAUS_ID, $HAUS_STRASSE, $HAUS_NUMMER ) = mysql_fetch_row ( $resultat ) ) {
			...
			$counter ++;
			if ($counter == 1) {
				echo "<tr class=\"zeile1\"><td>$HAUS_STRASSE</td><td>$HAUS_NUMMER</td><td>Details</td><td><a href='" . route('legacy::einheiten::index', ['einheit_raus' => 'einheit_kurz', 'haus_id' => $HAUS_ID]) . "'>Einheiten</a></td><td>Mieter</td><td>Ändern</td><td><a href='" . route('legacy::haeuserform::index', ['daten_rein' => 'aendern_liste']) . "'>Löschen</a></td></tr>\n";
			}
			if ($counter == 2) {
				echo "<tr class=\"zeile2\"><td>$HAUS_STRASSE</td><td>$HAUS_NUMMER</td><td>Details</td><td><a href='" . route('legacy::einheiten::index', ['einheit_raus' => 'einheit_kurz', 'haus_id' => $HAUS_ID]) . "'>Einheiten</a></td><td>Mieter</td><td>Ändern</td><td><a href='" . route('legacy::haeuserform::index', ['daten_rein' => 'aendern_liste']) . "'>Löschen</a></td></tr>\n";
				$counter = 0;
			}
		}
		echo "</table>";
