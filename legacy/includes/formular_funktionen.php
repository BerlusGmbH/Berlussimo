...
{
    $result = DB::select("SELECT id AS OBJEKT_ID FROM OBJEKT ORDER BY id DESC LIMIT 0,1");
...
{
    $result = DB::select("SELECT COUNT(OBJEKT_KURZNAME) AS ANZAHL FROM OBJEKT ORDER BY id WHERE OBJEKT_KURZNAME='$kurzname' && OBJEKT_AKTUELL=1");
    return $result[0]['ANZAHL'];
}
...
function neues_objekt_anlegen($objekt_kurzname, $eigentuemer)
{
    ...
    DB::insert("INSERT INTO OBJEKT (OBJEKT_DAT, id, OBJEKT_AKTUELL, OBJEKT_KURZNAME, EIGENTUEMER_PARTNER) VALUES (NULL,'$letzte_obj_id','1', '$objekt_kurzname', '$eigentuemer')");
...
{
    $result = DB::select("SELECT OBJEKT_DAT, id AS OBJEKT_ID, OBJEKT_AKTUELL, OBJEKT_KURZNAME FROM OBJEKT WHERE id='$obj_id' ORDER BY OBJEKT_DAT DESC LIMIT 0,1");
...
function objekt_update_kurzname($obj_dat, $obj_id, $obj_kurzname)
{
    ...
    if ($obj_kurzname != '') {
        ...
        DB::insert("INSERT INTO OBJEKT (OBJEKT_DAT, id, OBJEKT_AKTUELL, OBJEKT_KURZNAME) VALUES (NULL,'$obj_id','1', '$obj_kurzname')");
        $dat_dat_alt = $obj_dat;
