...
function personen_liste_alle()
{
    ...
    if ($numrows < 1) {
        ...
        while (list ($PERSON_DAT, $PERSON_ID, $PERSON_NACHNAME, $PERSON_VORNAME, $PERSON_GEBURTSTAG) = mysql_fetch_row($resultat)) {
            ...
            $delete_link = "<a class=\"table_links\" href='" . route('legacy::personen::index', ['anzeigen' => 'person_loeschen', 'person_dat' => $PERSON_DAT]) . "'>Löschen</a>";
            $aendern_link = "<a class=\"table_links\" href='" . route('legacy::personen::index', ['anzeigen' => 'person_aendern', 'person_id' => $PERSON_ID]) . "'>Ändern</a>";
            $mietvertrag_link = "";
            $haus_info = "";
            $haus_info_link = "";
    ...
    for ($i = 0; $i < $anzahl_einheiten; $i++) {
        ...
        if ($mietvertrag_exists == 0) {
            ...
            $leerstand [] = array(
                ...
                "EINHEIT_ID" => "" . $einheit_ids [$i] ['EINHEIT_ID'] . ""
            );
...
function person_aendern_in_db($person_id)
{
    ...
    $db_abfrage = "INSERT INTO PERSON (`PERSON_DAT`, `PERSON_ID`, `PERSON_NACHNAME`, `PERSON_VORNAME`, `PERSON_GEBURTSTAG`, `PERSON_AKTUELL`) VALUES (NULL, '$akt_person_id', '" . request()->input('person_nachname') . "', '" . request()->input('person_vorname') . "', date_format( '$gebdatum', '%Y-%m-%d' ), '1')";
    $resultat = mysql_query($db_abfrage) or die (mysql_error());
...
function person_in_db_eintragen()
{
    ...
    if ($person_status != "error") {
        ...
        $db_abfrage = "INSERT INTO PERSON (`PERSON_DAT`, `PERSON_ID`, `PERSON_NACHNAME`, `PERSON_VORNAME`, `PERSON_GEBURTSTAG`, `PERSON_AKTUELL`) VALUES (NULL, '$akt_person_id', '" . request()->input('person_nachname') . "', '" . request()->input('person_vorname') . "', '$gebdatum', '1')";
        $resultat = mysql_query($db_abfrage) or die (mysql_error());
