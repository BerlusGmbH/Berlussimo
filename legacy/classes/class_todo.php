...
class todo
{
    ...
    {
        $db_abfrage = "SELECT *, BAUSTELLEN_EXT.id AS ID FROM BAUSTELLEN_EXT WHERE AKTUELL='1' && AKTIV='$aktiv' ORDER BY BEZ, PARTNER_ID";
...
AND `AKTUELL` = '1' && ERLEDIGT='1' && UE_ID='0'";
    ...
    {
        $ee = new einheit ();
        $einheit = \App\Models\Einheiten::where('id', $einheit_id)->firstOrFail();
    ...
    function pdf_auftrag($id)
    {
        ...
        try {
            $partner_id = \App\Models\Auftraege::where('T_ID', $id)->first()->von->arbeitgeber()->first()->id;
        } catch(Exception $e) {
    ...
    {
        $db_abfrage = "SELECT id AS HAUS_ID FROM HAUS WHERE HAUS_AKTUELL='1' && HAUS_STRASSE='$haus_str' && HAUS_NUMMER='$haus_nr' && HAUS_PLZ='$haus_plz'";
        $resultat = DB::select($db_abfrage);
        if (!empty($resultat)) {
            return $resultat;
        }
