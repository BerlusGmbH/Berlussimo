 ...
 * 
 * @filesource   $HeadURL: http://192.168.2.52/svn/berlussimo_1/tags/02.11.2010 - Downloadversion 0.27/js/javascript.js $
 * @version      $Revision: 15 $
...
function gesamt_berechnen(){
	...
	//var anzahl_positionen = <?php echo count($_POST['positionen']); ?>;
	...
	document.getElementById('gesamt_errechnet').innerHTML = 'Netto ' + gesamt_bisher_netto_komma + ' �';
	...
	document.getElementById('gesamt_errechnet_brutto').innerHTML = 'Brutto ' + gesamt_bisher_brutto_komma + ' �';
	...
	document.getElementById('differenz').innerHTML = 'Differenz ' + differenz_komma + ' �';
...
/*<input type="button" name="button" onclick='activate(this.form.elements.["Stadt[]"]);' value="Ich habe alle besucht">  */
var marker = true;
function activate(field) {
    for(var i = 0; i < field.length; i++) field[i].checked = marker;
    marker = !marker;
}  
...
document.getElementById('g_netto_errechnet').innerHTML = g_netto_errechnet_ausgabe + '&nbsp�';
document.getElementById('durchschnitt_rabatt').innerHTML = durchschnitt_rabatt + '&nbsp%';
...
if(laenge>0){
	for(var i=0; i < laenge; i++) {
	var check_box_status = form.positionen_list[i].checked;
	...
	skonto_wert = parseInt(skonto_wert);
	*/
	pos_gesamt_netto = (mengen_wert*epreis_wert/100)*(100-rabatt_wert);
	...
	//var netto_new = netto.toFixed(2);
	//alert(pos_gesamt_netto + '  ' + pos_gesamt_br + ' ' + pos_gesamt_skontiert);

	/*Summen aller Zeilen im Pool bilden, wenn alle ausgew�hlt = summe_ausgewaehlt*/
 ...
 if(check_box_status){
	...
	/*var g_netto_errechnet_a = nummer_punkt2komma(g_netto_errechnet);
	var g_brutto_errechnet_a = nummer_punkt2komma(g_brutto_errechnet);
	var g_skonto_n_errechnet_a = nummer_punkt2komma(g_skonto_n_errechnet);	
	var g_skonto_betrag_errechnet_a = nummer_punkt2komma(g_brutto_errechnet);
	durchschnitt_rabatt = durchschnitt_rabatt/laenge;
	durchschnitt_rabatt_a = nummer_punkt2komma(durchschnitt_rabatt);
	*/
	
	/*Update von Ausgew�hlten Summen*/
	document.getElementById('g_netto_ausgewaehlt').innerHTML = g_netto_ausgewaehlt_a + '&nbsp�';
	document.getElementById('g_brutto_ausgewaehlt').innerHTML = g_brutto_ausgewaehlt_a + '&nbsp�';
	document.getElementById('g_skonto_nachlass').innerHTML = g_skonto_n_a + '&nbsp�';
	document.getElementById('g_skonto_betrag').innerHTML = g_skonto_betrag + '&nbsp�';
	
	/*Update von Ausgew�hlten Summen*/
	/*document.getElementById('g_netto_errechnet').innerHTML = g_netto_errechnet_a + '&nbsp�';
	document.getElementById('g_brutto_errechnet').innerHTML = g_brutto_errechnet_a + '&nbsp�';
	document.getElementById('durchschnitt_rabatt').innerHTML = durchschnitt_rabatt_a + '&nbsp%';
	*/
	
	/*Update von Ausgew�hlten Summen versteckte Felder f�r Weitergabe per FORM*/
	...
	*/
	...
	var check_box_status = form.positionen_list.checked;
	//alert(check_box_status);
	...
	var netto_wert = form.netto_feld.value;
	var mwst_wert = form.mwst_feld.value;
	var skonto_wert = form.skonto_feld.value;
	
	epreis_wert = epreis_wert.replace(",",".");
	form.epreis_feld.value = epreis_wert.replace(".",",");
	mengen_wert = mengen_wert.replace(",",".");
	form.mengen_feld.value = mengen_wert.replace(".",",");
	rabatt_wert = rabatt_wert.replace(",",".");
	rabatt_wert = rabatt_wert.replace(",",".");
	mwst_wert = parseInt(mwst_wert.replace(",","."));
	skonto_wert = skonto_wert.replace(",",".");
	...
	pos_gesamt_netto_a = pos_gesamt_netto.toFixed(2);
	pos_gesamt_netto_a = pos_gesamt_netto_a.replace(".",",");
	form.netto_feld.value = pos_gesamt_netto_a;
	//var netto_new = netto.toFixed(2);
	//alert(pos_gesamt_netto + '  ' + pos_gesamt_br + ' ' + pos_gesamt_skontiert);
 ...
 if(check_box_status){
	...
	g_brutto_ausgewaehlt_a = nummer_punkt2komma(g_brutto_ausgewaehlt);
	g_skonto_n_a = nummer_punkt2komma(g_skonto_n);
	/*g_skonto_betrag_a = nummer_punkt2komma(g_skonto_betrag);*/
...
document.getElementById('g_netto_errechnet').innerHTML = g_netto_errechnet_ausgabe + '&nbsp�';
document.getElementById('g_brutto_errechnet').innerHTML = g_brutto_errechnet_ausgabe + '&nbsp�';
document.getElementById('durchschnitt_rabatt').innerHTML = durchschnitt_rabatt + '&nbsp%';
document.getElementById('g_netto_ausgewaehlt').innerHTML = g_netto_ausgewaehlt_komma + '&nbsp�';
document.getElementById('g_brutto_ausgewaehlt').innerHTML = g_brutto_ausgewaehlt_komma + '&nbsp�';
document.getElementById('g_skonto_nachlass').innerHTML = g_skonto_nachlass_komma + '&nbsp�' + '(' + skonto_wert +'%)';
document.getElementById('g_skonto_betrag').innerHTML = g_skonto_betrag_komma + '&nbsp�';
...
bruttopreis = (nettopreis / parseInt(100)) * (parseInt(100) + parseInt(mwst_satz));
...
function prozentbetrag(vollbetrag, prozente_feld, feld){
	teilbetrag = (vollbetrag.replace(",",".")/100)*document.getElementById(prozente_feld).value.replace(",",".");
...
function prozente(vollbetrag, teilbetrag, feld){
	prozent = teilbetrag.replace(",",".")/(vollbetrag.replace(",",".")/100)
...
function count_auswahl(obj, max){
	var intGesamt = max;  // Gesamtanzahl Checkboxen, die gesetzt werden d�rfen
...
function remove_from_dd(list_id){
		...
		var selected_index = liste.options[liste.selectedIndex];
		liste.remove(selected_index);	
		if(liste.length == 0){
...
function staffel_berechnen(a_datum_feld, endjahr_feld, betrag_feld, prozent_betrag, mwst){
	...
	var prozent = proz_betrag.charAt(zeichen_l);
		...
		for(a=a_jahr;a<=endjahr;a++){
			...
			//alert(nur_proz);
			temp_betrag1 = parseFloat((temp_betrag)/100)*(100+parseFloat(nur_proz));
