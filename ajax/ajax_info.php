...
define ("BERLUS_PATH", ''); //wegen include bei AJAX anders
...
session_start();
header('Content-Type: text/plain; charset=ISO-8859-1'); //wichtig fÃ¼r die Umlaute in Dropdownfeldern
...
while ($row = mysql_fetch_assoc($result)) $rechnungs_positionen_arr[] = $row;
...
header('Content-Type: text/plain; charset=UFT-8'); //wichtig f€r die Umlaute in Dropdownfeldern
$belegnr = $_REQUEST["belegnr"];
$pos = $_REQUEST["pos"];
$artikel_nr= $_REQUEST["artikel_nr"];
$bez = $_REQUEST["bez"];
...
$belegnr=$_REQUEST["belegnr"];
$r->rechnung_grunddaten_holen($belegnr);
$buchungsbetrag = $_REQUEST["buchungsbetrag"]; //netto, brutto, skonto, keine summe oder betrag
...
$string = $_REQUEST["string"];
$string= utf8_decode($string);
$lieferant_id = $_REQUEST["l_id"]; //aktueller partner d.h. eigener preis
if(isset($string) && strlen($string) >0){
...
/*Betriebskostenabrechnung - L€schen von Buchungen zum Profil*/
case "buchung_raus":
